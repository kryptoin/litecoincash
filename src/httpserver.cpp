  ...
  for (const std::string &strAllow : gArgs.GetArgs("-rpcallowip")) {
    ...
    if (!subnet.IsValid()) {
      uiInterface.ThreadSafeMessageBox(
          strprintf(
              "Invalid -rpcallowip subnet specification: %s. Valid are a "
...
static bool ThreadHTTP(struct event_base *base, struct evhttp *http) {
  RenameThread("litecoincash-http");
...
static void HTTPWorkQueueRun(WorkQueue<HTTPClosure> *queue) {
  RenameThread("litecoincash-httpworker");
  queue->Run();
}
...
void StopHTTPServer() {
  ...
  if (eventBase) {
    ...
    if (threadResult.valid() && threadResult.wait_for(std::chrono::milliseconds(
                                    2000)) == std::future_status::timeout) {
