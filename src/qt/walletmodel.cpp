...
WalletModel::WalletModel(const PlatformStyle *platformStyle, CWallet *_wallet,
    ...
    : QObject(parent), wallet(_wallet), optionsModel(_optionsModel),
      ...
      recentRequestsTableModel(0), hiveTableModel(0),
  ...
  hiveTableModel = new HiveTableModel(platformStyle, wallet, this);
...
void WalletModel::updateTransaction() { fForceCheckBalanceChanged = true; }
  ...
  for (const SendCoinsRecipient &rcp : recipients) {
    ...
    if (rcp.paymentRequest.IsInitialized()) {
      ...
      CScript scriptPubKey =
          GetScriptForDestination(DecodeDestination(rcp.address.toStdString()));
...
HiveTableModel *WalletModel::getHiveTableModel() { return hiveTableModel; }
...
WalletModel::UnlockContext WalletModel::requestUnlock(bool hiveOnly) {
  ...
  return UnlockContext(this, valid,
                       was_locked && !fWalletUnlockWithoutTransactions);
}
...
void WalletModel::getOutputs(const std::vector<COutPoint> &vOutpoints,
  ...
  for (const COutPoint &outpoint : vOutpoints) {
    ...
    if (nDepth < 0)
      continue;
    COutput out(&it->second, outpoint.n, nDepth, true, true, true);
...
void WalletModel::getBCTs(
    std::vector<CBeeCreationTransactionInfo> &vBeeCreationTransactions,
    bool includeDeadBees) {
  if (wallet) {
    LOCK(wallet->cs_wallet);
    vBeeCreationTransactions =
        wallet->GetBCTs(includeDeadBees, true, Params().GetConsensus());
  }
}

bool WalletModel::createBees(int beeCount, bool communityContrib,
                             QWidget *parent, double beePopIndex) {
  ...
  if (!wallet->CreateBeeTransaction(
  ...
  CAmount totalAmount =
      wtxNew.GetDebit(ISMINE_ALL) - wtxNew.GetCredit(ISMINE_ALL);
  ...
  if (beePopIndex > 90)
    questionString.append(
        "<br /><span style='color:#aa0000;'><b>WARNING:</b> Global Index is "
        "high and bees may not be profitable. Please ensure you understand the "
        "consequences before proceeding.</span><br />");

  questionString.append("<br />");
  questionString.append("<b>" +
                        BitcoinUnits::formatHtmlWithUnit(
                            optionsModel->getDisplayUnit(), amountWithoutFees) +
                        "</b>");
  questionString.append(" to create " + QString::number(beeCount) + " bees");

  questionString.append("<hr /><span style='color:#aa0000;'>");
  questionString.append(
      BitcoinUnits::formatHtmlWithUnit(optionsModel->getDisplayUnit(), txFee));
  questionString.append("</span> ");
  questionString.append(tr("added as transaction fee"));
  questionString.append(
      " (" +
      QString::number((double)GetVirtualTransactionSize(*wtxNew.tx) / 1000) +
      " kB)");
  questionString.append("<hr />");

  questionString.append("Total Amount " +
                        BitcoinUnits::formatHtmlWithUnit(
                            optionsModel->getDisplayUnit(), totalAmount));
  ...
  SendConfirmationDialog confirmationDialog(tr("Confirm bee creation"),
                                            questionString);
  ...
  CValidationState state;
  if (!wallet->CommitTransaction(wtxNew, reservekeyChange, g_connman.get(),
                                 state)) {
    QMessageBox::critical(parent, tr("Error"),
                          "Bee creation error: " +
                              QString::fromStdString(state.GetRejectReason()));
  ...
  QMessageBox::information(parent, tr("Success"),
                           "Bees created! Bees will appear in your hive as "
                           "soon as the transaction appears in a block.");
  return true;
}
...
bool WalletModel::bumpFee(uint256 hash) {
  ...
  if (feebumper::CreateTransaction(wallet, hash, coin_control, 0, errors,
                                   old_fee, new_fee,
                                   mtx) != feebumper::Result::OK) {
  ...
  questionString.append(BitcoinUnits::formatHtmlWithUnit(
      getOptionsModel()->getDisplayUnit(), old_fee));
  questionString.append("</td></tr><tr><td>");
  questionString.append(tr("Increase:"));
  questionString.append("</td><td>");
  questionString.append(BitcoinUnits::formatHtmlWithUnit(
      getOptionsModel()->getDisplayUnit(), new_fee - old_fee));
  questionString.append("</td></tr><tr><td>");
  questionString.append(tr("New fee:"));
  questionString.append("</td><td>");
  questionString.append(BitcoinUnits::formatHtmlWithUnit(
      getOptionsModel()->getDisplayUnit(), new_fee));
  ...
  uint256 txid;
  if (feebumper::CommitTransaction(wallet, hash, std::move(mtx), errors,
...
OutputType WalletModel::getDefaultAddressType() const { return g_address_type; }
