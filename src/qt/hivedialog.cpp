...
HiveDialog::HiveDialog(const PlatformStyle *_platformStyle, QWidget *parent)
    : QDialog(parent), ui(new Ui::HiveDialog), columnResizingFixer(0), model(0),
  ...
  if (!_platformStyle->getImagesOnButtons())
    ...
    ui->createBeesButton->setIcon(
        _platformStyle->SingleColorIcon(":/icons/bee"));
...
void HiveDialog::setModel(WalletModel *_model) {
  ...
  if (_model && _model->getOptionsModel()) {
    _model->getHiveTableModel()->sort(HiveTableModel::Created,
                                      Qt::DescendingOrder);
    ...
    connect(_model, SIGNAL(encryptionStatusChanged(int)), this,
    ...
    columnResizingFixer = new GUIUtil::TableViewLastColumnResizingFixer(
        tableView, REWARDS_COLUMN_WIDTH, HIVE_COL_MIN_WIDTH, this);

    updateData(true);
  }
}

HiveDialog::~HiveDialog() { delete ui; }

void HiveDialog::setBalance(const CAmount &balance,
...
void HiveDialog::updateData(bool forceGlobalSummaryUpdate) {
  ...
  if (model && model->getHiveTableModel()) {
    model->getHiveTableModel()->updateBCTs(
        ui->includeDeadBeesCheckbox->isChecked());
    model->getHiveTableModel()->getSummaryValues(
        immature, mature, dead, blocksFound, cost, rewardsPaid, profit);
    ...
    if (dead == 0) {
      ...
      ui->deadLabelSpacer->changeSize(0, 0, QSizePolicy::Fixed,
                                      QSizePolicy::Fixed);
    } else {
      ui->deadLabel->setText(formatLargeNoLocale(dead));
      ui->deadLabel->show();
      ui->deadTitleLabel->show();
      ui->deadLabelSpacer->changeSize(
          ui->immatureLabelSpacer->geometry().width(), 0, QSizePolicy::Fixed,
          QSizePolicy::Fixed);
    }
    ...
    if (clientModel && clientModel->getNumConnections() == 0) {
      ...
      if (mature + immature == 0) {
        ...
        if (model->getEncryptionStatus() == WalletModel::Locked) {
          tooltip =
              "WARNING: Bees mature but not mining because wallet is locked";
  ...
  if (forceGlobalSummaryUpdate ||
      chainActive.Tip()->nHeight >= lastGlobalCheckHeight + 10) {
    ...
    if (!GetNetworkHiveInfo(globalImmatureBees, globalImmatureBCTs,
                            ...
                            potentialRewards, consensusParams, true)) {
      ui->globalHiveSummary->hide();
      ui->globalHiveSummaryError->show();
    } else {
      ui->globalHiveSummaryError->hide();
      ...
      if (globalImmatureBees == 0)
        ...
        ui->globalImmatureLabel->setText(
            formatLargeNoLocale(globalImmatureBees) + " (" +
            QString::number(globalImmatureBCTs) + " transactions)");

      if (globalMatureBees == 0)
        ui->globalMatureLabel->setText("0");
      else
        ui->globalMatureLabel->setText(
            formatLargeNoLocale(globalMatureBees) + " (" +
            QString::number(globalMatureBCTs) + " transactions)");
    ...
    ui->localHiveWeightLabel->setText(
        (mature == 0 || globalMatureBees == 0)
            ? "0"
            : QString::number(hiveWeight, 'f', 3));
    ui->hiveWeightPie->setValue(hiveWeight);

    beePopIndex =
        ((beeCost * globalMatureBees) / (double)potentialRewards) * 100.0;
    if (beePopIndex > 200)
      beePopIndex = 200;
  ...
  ui->blocksTillGlobalRefresh->setText(QString::number(
      10 - (chainActive.Tip()->nHeight - lastGlobalCheckHeight)));
}
...
void HiveDialog::on_includeDeadBeesCheckbox_stateChanged() { updateData(); }
...
void HiveDialog::on_retryGlobalSummaryButton_clicked() { updateData(true); }

void HiveDialog::on_refreshGlobalSummaryButton_clicked() { updateData(true); }
  ...
  if (model) {
    if (totalCost > model->getBalance()) {
      QMessageBox::critical(this, tr("Error"),
                            tr("Insufficient balance to create bees."));
      return;
    }
    ...
    model->createBees(ui->beeCountSpinner->value(),
                      clientModel->getOptionsModel()->getHiveContribCF(), this,
                      beePopIndex);
  }
}
...
void HiveDialog::initGraph() {
  ...
  dateTicker->setTickStepStrategy(
  ...
  connect(ui->beePopGraph->xAxis, SIGNAL(rangeChanged(QCPRange)),
          ui->beePopGraph->xAxis2, SLOT(setRange(QCPRange)));
  connect(ui->beePopGraph->yAxis, SIGNAL(rangeChanged(QCPRange)),
          ui->beePopGraph->yAxis2, SLOT(setRange(QCPRange)));
  connect(ui->beePopGraph, SIGNAL(mouseMove(QMouseEvent *)), this,
...
void HiveDialog::updateGraph() {
  ...
  globalMarkerLine->end->setCoords(
      now + consensusParams.nPowTargetSpacing / 2 * totalLifespan, global100);
...
void HiveDialog::onMouseMove(QMouseEvent *event) {
  ...
  graphMouseoverText->setText(
      xDateTime.toString("ddd d MMM") + " " + xDateTime.time().toString() +
