...
TransactionView::TransactionView(const PlatformStyle *platformStyle,
  ...
  typeWidget->addItem(
      tr("Hive bee creation"),
      TransactionFilterProxy::TYPE(TransactionRecord::HiveBeeCreation));

  typeWidget->addItem(
      tr("Hive community fund"),
      TransactionFilterProxy::TYPE(TransactionRecord::HiveCommunityFund));

  typeWidget->addItem(tr("Hivemined honey"), TransactionFilterProxy::TYPE(
                                                 TransactionRecord::HiveHoney));
  ...
  connect(search_widget, SIGNAL(textChanged(QString)), prefix_typing_delay,
          SLOT(start()));
