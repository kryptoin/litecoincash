...
BitcoinGUI::BitcoinGUI(const PlatformStyle *_platformStyle,
    ...
    : QMainWindow(parent), enableWallet(false), clientModel(0), walletFrame(0),
      ...
      labelWalletHDStatusIcon(0), connectionsControl(0), hiveStatusIcon(0),

      labelBlocksIcon(0), progressBarLabel(0), progressBar(0),
      progressDialog(0), appMenuBar(0), overviewAction(0), hiveAction(0),

      importPrivateKeyAction(0),

      historyAction(0), quitAction(0), sendCoinsAction(0),
  ...
  QString windowTitle = tr("Litecoin Cash - ");
  ...
  frameBlocksLayout->addStretch();
  frameBlocksLayout->addWidget(hiveStatusIcon);
  ...
  updateHiveStatusIcon(":/icons/hivestatus_disabled",
                       "The Hive is not enabled on the network");
}
...
void BitcoinGUI::createActions() {
  ...
  receiveCoinsAction->setStatusTip(
      tr("Request payments (generates QR codes and litecoincash: URIs)"));
  ...
  hiveAction = new QAction(platformStyle->SingleColorIcon(":/icons/bee"),
                           tr("The &Hive"), this);
  ...
  connect(hiveAction, SIGNAL(triggered()), this, SLOT(showNormalIfMinimized()));

  connect(hiveAction, SIGNAL(triggered()), this, SLOT(gotoHivePage()));
  ...
  signMessageAction->setStatusTip(tr(
      "Sign messages with your LitecoinCash addresses to prove you own them"));
  verifyMessageAction =
  ...
  showHelpMessageAction->setStatusTip(
      tr("Show the %1 help message to get a list with possible LitecoinCash "
         "command-line options")
          .arg(tr(PACKAGE_NAME)));

  importPrivateKeyAction =
      new QAction(platformStyle->TextColorIcon(":/icons/key"),
                  tr("&Import private key..."), this);
  importPrivateKeyAction->setToolTip(
      tr("Import a Litecoin or LitecoinCash private key"));
  ...
  if (walletFrame) {
    ...
    connect(importPrivateKeyAction, SIGNAL(triggered()), walletFrame,
            SLOT(importPrivateKey()));
  }
#endif
...
void BitcoinGUI::updateNetworkState() {
  ...
  if (clientModel->getNetworkActive()) {
    tooltip = tr("%n active connection(s) to LitecoinCash network", "", count) +
...
void BitcoinGUI::updateHiveStatusIcon(QString icon, QString tooltip) {
  QPixmap pixmap;
  pixmap.load(icon);
  hiveStatusIcon->setPixmap(
      pixmap.scaled(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
  hiveStatusIcon->setToolTip(tooltip);
}

void BitcoinGUI::setNumConnections(int count) { updateNetworkState(); }

void BitcoinGUI::setNetworkActive(bool networkActive) { updateNetworkState(); }
...
void BitcoinGUI::message(const QString &title, const QString &message,
  ...
  QString strTitle = tr("Litecoin Cash");
