...
uint256 SendCoins(CWallet &wallet, SendCoinsDialog &sendCoinsDialog,
                  const CTxDestination &address, CAmount amount, bool rbf) {
...
void TestGUI() {
  ...
  uint256 txid1 = SendCoins(wallet, sendCoinsDialog, CKeyID(), 5 * COIN, false);
  uint256 txid2 = SendCoins(wallet, sendCoinsDialog, CKeyID(), 10 * COIN, true);
  ...
  for (QWidget *widget : QApplication::topLevelWidgets()) {
    if (widget->inherits("ReceiveRequestDialog")) {
      ...
      QVERIFY(paymentTextList.at(1).indexOf(QString("URI: litecoincash:")) !=
              -1);
      QVERIFY(paymentTextList.at(2).indexOf(QString("Address:")) != -1);
      QCOMPARE(paymentTextList.at(3),
               QString("Amount: 0.0000001 ") +
...
void WalletTests::walletTests() { TestGUI(); }
