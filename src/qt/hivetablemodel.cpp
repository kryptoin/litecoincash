...
HiveTableModel::HiveTableModel(const PlatformStyle *_platformStyle,
                               CWallet *wallet, WalletModel *parent)
    : platformStyle(_platformStyle), QAbstractTableModel(parent),
      walletModel(parent) {
  Q_UNUSED(wallet);

  columns << tr("Created") << tr("Bee count") << tr("Bee status")
          << tr("Estimated time until status change") << tr("Bee cost")
          << tr("Rewards earned");
...
void HiveTableModel::updateBCTs(bool includeDeadBees) {
  if (walletModel) {
    ...
    QMetaObject::invokeMethod(walletModel, "newHiveSummaryAvailable",
                              Qt::QueuedConnection);
  }
}

void HiveTableModel::getSummaryValues(int &_immature, int &_mature, int &_dead,
                                      int &_blocksFound, CAmount &_cost,
                                      CAmount &_rewardsPaid, CAmount &_profit) {
  _immature = immature;
  _mature = mature;
...
QVariant HiveTableModel::data(const QModelIndex &index, int role) const {
  ...
  if (role == Qt::DisplayRole || role == Qt::EditRole) {
    switch (index.column()) {
    case Created:
      return (rec->time == 0) ? "Not in chain yet"
                              : GUIUtil::dateTimeStr(rec->time);
    case Count:
    ...
    case EstimatedTime: {
      ...
      if (rec->beeStatus == "immature") {
        int blocksTillMature =
            rec->blocksLeft - Params().GetConsensus().beeLifespanBlocks;
        status =
            "Matures in " + QString::number(blocksTillMature) + " blocks (" +
            secondsToString(blocksTillMature *
                            Params().GetConsensus().nPowTargetSpacing / 2) +
            ")";
      } else if (rec->beeStatus == "mature")
        status =
            "Expires in " + QString::number(rec->blocksLeft) + " blocks (" +
            secondsToString(rec->blocksLeft *
                            Params().GetConsensus().nPowTargetSpacing / 2) +
            ")";
      return status;
    }
    ...
    if (index.column() == Cost || index.column() == Rewards ||
        index.column() == Count)
    ...
    const CBeeCreationTransactionInfo *rec = &list[index.row()];
    if (index.column() == Status) {
      QString iconStr = ":/icons/beestatus_dead";
...
bool HiveTableModel::setData(const QModelIndex &index, const QVariant &value,
                             int role) {
  return true;
}

QVariant HiveTableModel::headerData(int section, Qt::Orientation orientation,
                                    int role) const {
...
void HiveTableModel::sort(int column, Qt::SortOrder order) {
  ...
  qSort(list.begin(), list.end(),
        CBeeCreationTransactionInfoLessThan(column, order));
  Q_EMIT dataChanged(
      index(0, 0, QModelIndex()),
      index(list.size() - 1, NUMBER_OF_COLUMNS - 1, QModelIndex()));
}

bool CBeeCreationTransactionInfoLessThan::operator()(
    CBeeCreationTransactionInfo &left,
    CBeeCreationTransactionInfo &right) const {
...
QString HiveTableModel::secondsToString(qint64 seconds) {
  ...
  return QString("%1 days %2 hrs %3 mins")
      .arg(days)
      .arg(t.hour())
      .arg(t.minute());
}
