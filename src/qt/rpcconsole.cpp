...
void RPCExecutor::request(const QString &command) {
  try {
      ...
      Q_EMIT reply(
          ...
          QString(
              ("\n"
               ...
               "This console can also accept RPC commands using parenthesized "
               "syntax.\n"
               "   example:    getblockhash(0)\n\n"

               "Commands may be nested when specified with the parenthesized "
               "syntax.\n"
               "   example:    getblock(getblockhash(0) 1)\n\n"

               "A space or a comma can be used to delimit arguments for either "
               "syntax.\n"
               "   example:    getblockhash 0\n"
               "               getblockhash,0\n\n"

               "Named results can be queried with a non-quoted key string in "
               "brackets.\n"
               "   example:    getblock(getblockhash(0) true)[tx]\n\n"

               "Results without keys can be queried using an integer in "
               "brackets.\n"
...

    {
      ...
      Q_EMIT reply(RPCConsole::CMD_ERROR, QString::fromStdString(message) +
                                              " (code " +
                                              QString::number(code) + ")");
...
void RPCConsole::clear(bool clearHistory) {
  ...
  message(
      ...
      (tr("Welcome to the %1 RPC console.").arg(tr(PACKAGE_NAME)) + "<br>" +
       ...
       tr("WARNING: Scammers have been active, telling users to type commands "
          "here, stealing their wallet contents. Do not use this console "
          "without fully understanding the ramifications of a command.") +
       "</span>"),
      true);
}
