...
void OverviewPage::setWalletModel(WalletModel *model) {
  ...
  if (model && model->getOptionsModel()) {
    ...
    connect(model, SIGNAL(newHiveSummaryAvailable()), this,
            SLOT(updateHiveSummary()));
...
void OverviewPage::updateHiveSummary() {
  if (walletModel && walletModel->getHiveTableModel()) {
    ...
    walletModel->getHiveTableModel()->getSummaryValues(
...
void OverviewPage::on_beeButton_clicked() { Q_EMIT beeButtonClicked(); }
