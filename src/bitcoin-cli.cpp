...
std::string HelpMessageCli() {
  ...
  strUsage += HelpMessageOpt(
      "-stdin",
      _("Read extra arguments from standard input, one per line until "
        "EOF/Ctrl-D (recommended for sensitive information such as "
        "passphrases).  When combined with -stdinrpcpass, the first line from "
        "standard input is used for the RPC password."));
  strUsage += HelpMessageOpt(
      "-rpcwallet=<walletname>",
      _("Send RPC for non-default wallet on RPC server (argument is wallet "
        "filename in litecoincashd directory, required if litecoincashd/-Qt "
  ...
  if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
    ...
    if (!gArgs.IsArgSet("-version")) {
      strUsage +=
          ...
          "  litecoincash-cli [options] -named <command> [name=value] ... " +
          strprintf(_("Send command to %s (with named arguments)"),
                    _(PACKAGE_NAME)) +
          "\n" + "  litecoincash-cli [options] help                " +
          _("List commands") + "\n" +
          "  litecoincash-cli [options] help <command>      " +
    ...
    if (!GetAuthCookie(&strRPCUserColonPass)) {
      throw std::runtime_error(strprintf(
          _("Could not locate RPC credentials. No authentication cookie could "
            "be found, and RPC password is not set.  See -rpcpassword and "
            "-stdinrpcpass.  Configuration file: (%s)"),
