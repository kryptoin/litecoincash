...
std::string HelpMessage(HelpMessageMode mode) {
  ...
  strUsage += HelpMessageOpt(
      "-debuglogfile=<file>",
      strprintf(
          _("Specify location of debug log file: this can be an absolute path "
            "or a path relative to the data directory (default: %s)"),
  ...
  strUsage += HelpMessageOpt(
      "-addnode=<ip>",
      _("Add a node to connect to and attempt to keep the connection open (see "
  ...
  strUsage += HelpMessageOpt(
      "-connect=<ip>",
      _("Connect only to the specified node(s); -connect=0 disables automatic "
  ...
  strUsage += HelpMessageOpt(
      ...
      strprintf(
          _("Maximum number of automatic outgoing connections (default: %u)"),
  ...
  strUsage += HelpMessageOpt(
      "-rialto",
      strprintf(_("Support Rialto message propagration (default: %u)"),
                DEFAULT_RIALTO_SUPPORT));
  ...
  strUsage += HelpMessageOpt(
      ...
      _("Whitelist peers connecting from the given IP address (e.g. 1.2.3.4) "
        ...
        "times.") +
          " " +
          _("Whitelisted peers cannot be DoS banned and their transactions are "
            "always relayed, even if they are already in the mempool, useful "
            "e.g. for a gateway"));
  strUsage += HelpMessageOpt(
      "-maxuploadtarget=<n>",
  ...
  strUsage += HelpMessageOpt(
      "-rpcserialversion",
      strprintf(
          _("Sets the serialization of raw transaction or block hex returned "
            "in non-verbose mode, non-segwit(0) or segwit(1) (default: %d)"),
  ...
  strUsage += HelpMessageOpt(
      "-hivecheckdelay=<ms>",
      strprintf(
          _("Time between Hive checks in ms. This should be left at default "
            "unless performance degradation is observed (default: %u)"),
          DEFAULT_HIVE_CHECK_DELAY));
  strUsage += HelpMessageOpt(
      "-hivecheckthreads=<threads>",
      strprintf(
          _("Number of threads to use when checking bees, -1 for all available "
            "cores, or -2 for one less than all available cores (default: %u)"),
          DEFAULT_HIVE_THREADS));
  strUsage += HelpMessageOpt(
      "-hiveearlyabort",
      strprintf(_("Abort Hive checking as quickly as possible when a new block "
                  "comes in. This should be left enabled unless performance "
                  "degradation is observed. (default: %u)"),
                DEFAULT_HIVE_EARLY_OUT));

  strUsage += HelpMessageOpt(
      "-powalgo=sha256d|minotaurx",
      strprintf(
          _("Default pow mining algorithm. Miners who can't easily adjust "
            "their getblocktemplate calls should use this argument to set "
            "their preferred mining algorithm. (default: %s)"),
          DEFAULT_POW_TYPE));
  return strUsage;
}
...
std::string LicenseInfo() {
  ...
  return CopyrightHolders(
             strprintf(_("Copyright (C) %i-%i"), 2011, COPYRIGHT_YEAR) + " ") +
         "\n" + "\n" +
...
bool AppInitParameterInteraction() {
  ...
  if (gArgs.IsArgSet("-debug")) {
    ...
    if (std::none_of(categories.begin(), categories.end(), [](std::string cat) {
          return cat == "0" || cat == "none";
        })) {
  ...
  if ((!fEnableReplacement) && gArgs.IsArgSet("-mempoolreplacement")) {
    std::string strReplacementModeList =
        gArgs.GetArg("-mempoolreplacement", "");
...
bool AppInitMain() {
  ...
  if (gArgs.IsArgSet("-datadir") &&
    ...
    LogPrintf("Warning: relative datadir option '%s' specified, which will be "
              "interpreted relative to the "
              "current working directory '%s'. This is fragile, because if "
              "litecoincash is started in the future "
              "from a different location, it will be unable to locate the "
              "current data files. There could "
              "also be data loss if litecoincash is started while in a "
              "temporary directory.\n",
  ...
  nBlockTreeDBCache =
      std::min(nBlockTreeDBCache, (gArgs.GetBoolArg("-txindex", DEFAULT_TXINDEX)
  ...
  while (!fLoaded && !fRequestShutdown) {
    ...
    do {
      try {
        ...
        pwhitepages.reset(
            new CRialtoWhitePagesDB("whitepages", 2 * 1048576, false, fReset));

        pmynicks.reset();

        pmynicks.reset(
            new CRialtoWhitePagesDB("mynicks", 1048576, false, fReset));

        pblockednicks.reset();

        pblockednicks.reset(
            new CRialtoWhitePagesDB("blocklist", 1048576, false, fReset));
        ...
        pcoinsdbview.reset(new CCoinsViewDB(nCoinDBCache, false,
        ...
        if (!is_coinsview_empty) {
          ...
          if (fHavePruned && gArgs.GetArg("-checkblocks", DEFAULT_CHECKBLOCKS) >
                                 MIN_BLOCKS_TO_KEEP) {
  ...
  connOptions.nMaxOutbound =
      std::min((int)gArgs.GetArg("-maxoutboundconnections",
                                 DEFAULT_MAX_OUTBOUND_CONNECTIONS),
