...
SOCKET CreateSocket(const CService &addrConnect) {
  ...
  if (!SetSocketNonBlocking(hSocket, true)) {
    CloseSocket(hSocket);
    LogPrintf("ConnectSocketDirectly: Setting socket to non-blocking failed, "
              "error %s\n",
...
bool ConnectThroughProxy(const proxyType &proxy, const std::string &strDest,
  ...
  if (!proxy.randomize_credentials) {
    ...
    if (Lookup(strDest.c_str(), addrResolved, port, true)) {
      if (addrResolved.IsValid() && addrResolved.IsRoutable()) {
        if (!ReverseDNSValidation(addrResolved)) {
          LogPrint(
              BCLog::NET,
              "Proxy connection to %s:%d rejected (no valid reverse DNS)\n",
              strDest, port);
          return false;
        }
      }
    } else {
      LogPrint(BCLog::NET, "Proxy destination %s:%d could not be resolved\n",
               strDest, port);
      return false;
    }
  } else {
    LogPrint(BCLog::NET,
             "Skipping DNS validation for randomized proxy target %s:%d\n",
             strDest, port);
  }
