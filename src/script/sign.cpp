...
bool ProduceSignature(const BaseSignatureCreator &creator,
  ...
  if (solved && whichType == TX_WITNESS_V0_KEYHASH) {
    ...
    txnouttype subType;
    solved = solved &&
             SignStep(creator, witnessscript, result, subType,
                      SIGVERSION_WITNESS_V0) &&
...
bool IsSolvable(const CKeyStore &store, const CScript &script) {
  ...
  static_assert(STANDARD_SCRIPT_VERIFY_FLAGS & SCRIPT_VERIFY_WITNESS_PUBKEYTYPE,
                "IsSolvable requires standard script flags to include "
                "WITNESS_PUBKEYTYPE");
  if (ProduceSignature(creator, script, sigs)) {
    assert(VerifyScript(sigs.scriptSig, script, &sigs.scriptWitness,
                        STANDARD_SCRIPT_VERIFY_FLAGS, creator.Checker()));
    return true;
  }
  return false;
