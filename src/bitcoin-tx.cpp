...
static int AppInitRawTx(int argc, char *argv[]) {
  ...
  if (argc < 2 || gArgs.IsArgSet("-?") || gArgs.IsArgSet("-h") ||
      gArgs.IsArgSet("-help")) {
    std::string strUsage =
        strprintf(_("%s litecoincash-tx utility version"), _(PACKAGE_NAME)) +
        " " + FormatFullVersion() + "\n\n" + _("Usage:") + "\n" +
        "  litecoincash-tx [options] <hex-tx> [commands]  " +
        _("Update hex-encoded litecoincash transaction") + "\n" +
        "  litecoincash-tx [options] -create [commands]   " +
        _("Create hex-encoded litecoincash transaction") + "\n" + "\n";
    ...
    strUsage += HelpMessageOpt("outpubkey=VALUE:PUBKEY[:FLAGS]",
                               _("Add pay-to-pubkey output to TX") + ". " +
                                   _("Optionally add the \"W\" flag to produce "
                                     "a pay-to-witness-pubkey-hash output") +
                                   ". " +
                                   _("Optionally add the \"S\" flag to wrap "
                                     "the output in a pay-to-script-hash."));
    ...
    strUsage += HelpMessageOpt("outscript=VALUE:SCRIPT[:FLAGS]",
                               _("Add raw script output to TX") + ". " +
                                   _("Optionally add the \"W\" flag to produce "
                                     "a pay-to-witness-script-hash output") +
                                   ". " +
                                   _("Optionally add the \"S\" flag to wrap "
                                     "the output in a pay-to-script-hash."));
    strUsage += HelpMessageOpt(
    ...
    strUsage +=
        HelpMessageOpt("sign=SIGHASH-FLAGS",
                       _("Add zero or more signatures to transaction") + ". " +
                           ...
                           _("See signrawtransaction docs for format of "
