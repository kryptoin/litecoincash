...
std::string CTxOut::ToString() const {
  return strprintf("CTxOut(nValue=%d.%07d, scriptPubKey=%s)", nValue / COIN,
...
bool CTransaction::IsBCT(const Consensus::Params &consensusParams,
                         CScript scriptPubKeyBCF, CAmount *beeFeePaid,
                         CScript *scriptPubKeyHoney) const {
  bool isBCT = CScript::IsBCTScript(vout[0].scriptPubKey, scriptPubKeyBCF,
                                    scriptPubKeyHoney);
...
bool CTransaction::IsNCT(const Consensus::Params &consensusParams,
                         CScript scriptPubKeyNCF, std::string *pubKey,
  ...
  if (vout[1].scriptPubKey.size() < 40)

    return false;

  if (vout[1].scriptPubKey[0] != OP_RETURN ||
      vout[1].scriptPubKey[35] != OP_NICK_CREATE)
    return false;
  ...
  std::string n = std::string(vout[1].scriptPubKey.begin() + 37,
                              vout[1].scriptPubKey.end());
