...
BOOST_AUTO_TEST_CASE(netbase_properties) {
  ...
  BOOST_CHECK(
      CreateInternal("FD6C:E9FE:4549:edb1:8e4:3588:e546:35ca").IsInternal());
...
BOOST_AUTO_TEST_CASE(netbase_splithost) {
  ...
  BOOST_CHECK(
      TestSplitHost("[::ffff:127.0.0.1]:62458", "::ffff:127.0.0.1", 62458));
...
BOOST_AUTO_TEST_CASE(netbase_lookupnumeric) {
  ...
  BOOST_CHECK(TestParse("[fd6d:e9fe:4549:1:2:3:4:5]",
                        "[fd6d:e9fe:4549:1:2:3:4:5]:65535"));
}
...
BOOST_AUTO_TEST_CASE(netbase_getgroup) {
  ...
  BOOST_CHECK(ResolveIP("1.2.3.4").GetGroup() ==
              std::vector<unsigned char>({(unsigned char)NET_IPV4, 1, 2}));
  ...
  BOOST_CHECK(ResolveIP("FD87:D87E:EB43:edb1:8e4:3588:e546:35ca").GetGroup() ==
              std::vector<unsigned char>({(unsigned char)NET_TOR, 239}));

  BOOST_CHECK(ResolveIP("2001:470:abcd:9999:9999:9999:9999:9999").GetGroup() ==
              std::vector<unsigned char>(
                  {(unsigned char)NET_IPV6, 32, 1, 4, 112, 175}));

  BOOST_CHECK(
      ResolveIP("2001:2001:9999:9999:9999:9999:9999:9999").GetGroup() ==
      std::vector<unsigned char>({(unsigned char)NET_IPV6, 32, 1, 32, 1}));
