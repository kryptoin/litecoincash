...
BOOST_AUTO_TEST_CASE(MempoolIndexingTest) {
  ...
  sortedOrder[0] = tx3.GetHash().ToString();

  sortedOrder[1] = tx5.GetHash().ToString();

  sortedOrder[2] = tx1.GetHash().ToString();

  sortedOrder[3] = tx4.GetHash().ToString();
...
BOOST_AUTO_TEST_CASE(MempoolAncestorIndexingTest) {
  ...
  std::vector<std::string> sortedOrder;
  sortedOrder.resize(5);
  sortedOrder[0] = tx2.GetHash().ToString();

  sortedOrder[1] = tx4.GetHash().ToString();
...
BOOST_AUTO_TEST_CASE(MempoolSizeLimitTest) {
  ...
  CFeeRate maxFeeRateRemoved(25000, GetVirtualTransactionSize(tx3) +
