...
BOOST_AUTO_TEST_CASE(util_IsHexNumber) {
  ...
  BOOST_CHECK(!IsHexNumber(""));

  BOOST_CHECK(!IsHexNumber("0x"));

  BOOST_CHECK(!IsHexNumber("0x0 "));

  BOOST_CHECK(!IsHexNumber(" 0x0"));

  BOOST_CHECK(!IsHexNumber("0x 0"));

  BOOST_CHECK(!IsHexNumber(" "));

  BOOST_CHECK(!IsHexNumber("0x0ga"));

  BOOST_CHECK(!IsHexNumber("x0"));

  BOOST_CHECK(!IsHexNumber("0x0x00"));
}
...
BOOST_AUTO_TEST_CASE(test_ParseInt32) {
  ...
  BOOST_CHECK(ParseInt32("01234", &n) && n == 1234);
  ...
  BOOST_CHECK(!ParseInt32("0x1", &n));

  BOOST_CHECK(!ParseInt32("0x1", &n));

  const char test_bytes[] = {'1', 0, '1'};
  std::string teststr(test_bytes, sizeof(test_bytes));
  BOOST_CHECK(!ParseInt32(teststr, &n));
...
BOOST_AUTO_TEST_CASE(test_ParseInt64) {
  ...
  BOOST_CHECK(ParseInt64("01234", &n) && n == 1234LL);
  ...
  BOOST_CHECK(!ParseInt64("0x1", &n));

  const char test_bytes[] = {'1', 0, '1'};
  std::string teststr(test_bytes, sizeof(test_bytes));
  BOOST_CHECK(!ParseInt64(teststr, &n));
...
BOOST_AUTO_TEST_CASE(test_ParseUInt32) {
  ...
  BOOST_CHECK(ParseUInt32("01234", &n) && n == 1234);
  ...
  BOOST_CHECK(!ParseUInt32("0x1", &n));

  BOOST_CHECK(!ParseUInt32("0x1", &n));

  const char test_bytes[] = {'1', 0, '1'};
  std::string teststr(test_bytes, sizeof(test_bytes));
  BOOST_CHECK(!ParseUInt32(teststr, &n));
...
BOOST_AUTO_TEST_CASE(test_ParseUInt64) {
  ...
  BOOST_CHECK(ParseUInt64("01234", &n) && n == 1234LL);
  ...
  BOOST_CHECK(!ParseUInt64("0x1", &n));

  const char test_bytes[] = {'1', 0, '1'};
  std::string teststr(test_bytes, sizeof(test_bytes));
  BOOST_CHECK(!ParseUInt64(teststr, &n));
...
BOOST_AUTO_TEST_CASE(test_ParseDouble) {
  ...
  BOOST_CHECK(ParseDouble("01234", &n) && n == 1234.0);
  ...
  BOOST_CHECK(!ParseDouble("0x1", &n));

  const char test_bytes[] = {'1', 0, '1'};
  std::string teststr(test_bytes, sizeof(test_bytes));
  BOOST_CHECK(!ParseDouble(teststr, &n));
...
BOOST_AUTO_TEST_CASE(test_FormatParagraph) {
  ...
  BOOST_CHECK_EQUAL(
      FormatParagraph(
          "This_is_a_very_long_test_string_without_any_spaces_so_it_should_"
          "just_get_returned_as_is_despite_the_length until it gets here",
          79),
...
BOOST_AUTO_TEST_CASE(test_LockDirectory) {
  ...
  pid_t pid = fork();
  if (!pid) {
    BOOST_CHECK_EQUAL(close(fd[1]), 0);

    TestOtherProcess(dirname, lockname, fd[0]);
  }
  BOOST_CHECK_EQUAL(close(fd[0]), 0);
