...
void CreateCreditAndSpend(const CKeyStore &keystore, const CScript &outscript,
  ...
  bool ret =
      SignSignature(keystore, *output, inputm, 0, SIGHASH_ALL | SIGHASH_FORKID);
...
BOOST_AUTO_TEST_CASE(test_big_witness_transaction) {
  ...
  for (uint32_t i = 0; i < mtx.vin.size(); i++) {
    ...
    CScriptCheck check(coins[tx.vin[i].prevout.n].out, tx, i,
...
BOOST_AUTO_TEST_CASE(test_witness) {
  ...
  CreateCreditAndSpend(
      keystore,
      GetScriptForDestination(CScriptID(GetScriptForWitness(scriptPubkey1))),
      output1, input1);
  CreateCreditAndSpend(
      keystore,
      GetScriptForDestination(CScriptID(GetScriptForWitness(scriptPubkey2))),
      output2, input2);
  ...
  CreateCreditAndSpend(
      keystore,
      GetScriptForDestination(CScriptID(GetScriptForWitness(scriptPubkey1L))),
      output1, input1, false);
  CreateCreditAndSpend(
      keystore,
      GetScriptForDestination(CScriptID(GetScriptForWitness(scriptPubkey2L))),
  ...
  CreateCreditAndSpend(
      keystore,
      GetScriptForDestination(CScriptID(GetScriptForWitness(scriptMulti))),
      output1, input1, false);
  CheckWithFlag(output1, input1, SCRIPT_VERIFY_P2SH, true);
  CheckWithFlag(output1, input1, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS,
                false);
  ...
  t.vout[0].scriptPubKey =
      CScript() << OP_RETURN
                << ParseHex(
                       ...
                       "0ea1f61deb649f6bc3f4cef3804678afdb0fe5548271967f1a67130"
                       "b7105cd6a828e03909a67962e0ea1f61deb649f6bc3f4cef38");
  ...
  t.vout[0].scriptPubKey =
      CScript() << OP_RETURN
                << ParseHex(
                       "04678afdb0fe5548271967f1a67130b7105cd6a828e03909a67962e"
                       "0ea1f61deb649f6bc3f4cef3804678afdb0fe5548271967f1a67130"
                       "b7105cd6a828e03909a67962e0ea1f61deb649f6bc3f4cef3800");
