...
bool GetWalletAddressesForKey(CWallet *const pwallet, const CKeyID &keyid,
  ...
  if (!fLabelFound) {
    strAddr = EncodeDestination(
        GetDestinationForKey(key.GetPubKey(), g_address_type));
  }
  return fLabelFound;
}
  ...
  bool fRescan = true;
  {
    ...
    if (fWalletUnlockWithoutTransactions)
      ...
      throw JSONRPCError(
          ...
          "Wallet is unlocked for Hive Mining and Rialto Messaging only.");
...
UniValue abortrescan(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 0)
    throw std::runtime_error("abortrescan\n"
                             "\nStops current wallet rescan triggered by an "
...
void ImportScript(CWallet *const pwallet, const CScript &script,
  ...
  if (!pwallet->HaveWatchOnly(script) && !pwallet->AddWatchOnly(script, 0)) {
...
UniValue importaddress(const JSONRPCRequest &request) {
...

  {
    ...
    if (IsValidDestination(dest)) {
      ...
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                         "Invalid LitecoinCash address or script");
    }
  }
  if (fRescan) {
    pwallet->RescanFromTime(TIMESTAMP_MIN, reserver, true);
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        HelpExampleCli("removeprunedfunds",
                       "\"c54357a1ff9f4e792198e75c01fc633acc6d093abd67ec1849596"
                       "637c3457bf2\"") +
        "\nAs a JSON-RPC call\n" +
...
UniValue importwallet(const JSONRPCRequest &request) {
  ...
  pwallet->RescanFromTime(nTimeBegin, reserver, false);
  pwallet->MarkDirty();
...
UniValue dumpprivkey(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"   (string, required) The litecoincash address for the "
        "private key\n"
        "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "BIP173 addresses, etc. may not be added automatically by "
        "importwallet.\n"
        ...
        "absolute or relative to litecoincashd)\n"
        "\nResult:\n"
        "{                           (json object)\n"
  ...
  if (boost::filesystem::exists(filepath)) {
    throw JSONRPCError(RPC_INVALID_PARAMETER,
                       filepath.string() +
                           " already exists. If you are sure this is what you "
  ...
  for (const CScriptID &scriptid : scripts) {
    ...
    if (pwallet->GetCScript(scriptid, script)) {
      file << strprintf("%s %s script=1", HexStr(script.begin(), script.end()),
                        create_time);
...
UniValue ProcessImport(CWallet *const pwallet, const UniValue &data,
  ...
  try {
    ...
    if (!isP2SH && (keys.size() > 1 || pubKeys.size() > 1)) {
      throw JSONRPCError(RPC_INVALID_PARAMETER,
    ...
    if (isP2SH) {
      ...
      if (::IsMine(*pwallet, redeemDestination) == ISMINE_SPENDABLE) {
        throw JSONRPCError(RPC_WALLET_ERROR,
                           "The wallet already contains the private key for "
                           "this address or script");
      }
...
UniValue importmulti(const JSONRPCRequest &mainRequest) {
  ...
  if (mainRequest.fHelp || mainRequest.params.size() < 1 ||
    ...
    throw std::runtime_error(
        ...
        "(string / json, required) Type of scriptPubKey (string for script, "
        "json for address)\n"
        "      \"timestamp\": timestamp | \"now\"                        , "
        "(integer / string, required) Creation time of the key in seconds "
        "since epoch (Jan 1 1970 GMT),\n"
        "                                                              or the "
        "string \"now\" to substitute the current synced blockchain time. The "
        "timestamp of the oldest\n"
        "                                                              key "
        "will determine how far back blockchain rescans need to begin for "
        "missing wallet transactions.\n"
        "                                                              \"now\" "
        "can be specified to bypass scanning, for keys which are known to "
        "never have been used, and\n"
        "                                                              0 can "
        "be specified to scan the entire blockchain. Blocks up to 2 hours "
        "before the earliest key\n"
        "                                                              "
        "creation time of all keys being imported by the importmulti call will "
        "be scanned.\n"
        "      \"redeemscript\": \"<script>\"                            , "
        "(string, optional) Allowed only if the scriptPubKey is a P2SH address "
        "or a P2SH scriptPubKey\n"
        "      \"pubkeys\": [\"<pubKey>\", ... ]                         , "
        "(array, optional) Array of strings giving pubkeys that must occur in "
        "the output or redeemscript\n"
        "      \"keys\": [\"<key>\", ... ]                               , "
        "(array, optional) Array of strings giving private keys whose "
        "corresponding public keys must occur in the output or redeemscript\n"
        "      \"internal\": <true>                                    , "
        "(boolean, optional, default: false) Stating whether matching outputs "
        "should be treated as not incoming payments\n"
        "      \"watchonly\": <true>                                   , "
        "(boolean, optional, default: false) Stating whether matching outputs "
        "should be considered watched even when they're not spendable, only "
        "allowed if keys are empty\n"
        "      \"label\": <label>                                      , "
        "(string, optional, default: '') Label to assign to the address (aka "
        "account name, for now), only allowed with internal=false\n"
        "    }\n"
        "  ,...\n"
        "  ]\n"
        "2. options                 (json, optional)\n"
        "  {\n"
        "     \"rescan\": <false>,         (boolean, optional, default: true) "
        "Stating if should rescan the blockchain after all imports\n"
        "  }\n"
        "\nNote: This call can take minutes to complete if rescan is true, "
        "during that time, other rpc calls\n"
        "may report that the imported keys, addresses or scripts exists but "
        ...
        "\nResponse is an array with the same size as the input that has the "
        "execution result :\n"
  ...
  if (fRescan && fRunScan && requests.size()) {
    int64_t scannedTime =
        pwallet->RescanFromTime(nLowestTimestamp, reserver, true);
    pwallet->ReacceptWalletTransactions();
          ...
          result.pushKV(
              ...
              JSONRPCError(
                  ...
                  strprintf("Rescan failed for key with creation timestamp %d. "
                            ...
                            "litecoincashd log for details) and could "
