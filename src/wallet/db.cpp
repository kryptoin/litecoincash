...
bool CDBEnv::Open(const fs::path &pathIn, bool retry) {
  ...
  if (!LockDirectory(pathIn, ".walletlock")) {
    LogPrintf("Cannot obtain a lock on wallet directory %s. Another instance "
              "of litecoincash may be using it.\n",
              strPath);
    return false;
  }
...
bool CDB::VerifyEnvironment(const std::string &walletFile,
  ...
  if (walletFile != (walletDir / walletFile).stem().string() +
                        (walletDir / walletFile).extension().string()) {
...
CDB::CDB(CWalletDBWrapper &dbw, const char *pszMode, bool fFlushOnCloseIn)
...

  {
    ...
    if (pdb == nullptr) {
      ...
      ret = pdb_temp->open(nullptr,

                           fMockDb ? nullptr : strFilename.c_str(),

                           fMockDb ? strFilename.c_str() : "main",
