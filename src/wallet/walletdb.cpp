...
bool ReadKeyValue(CWallet *pwallet, CDataStream &ssKey, CDataStream &ssValue,
  ...
  try {
    ...
    if (strType == "name") {
      ...
      if (!(CheckTransaction(*wtx.tx, state) && (wtx.GetHash() == hash) &&
            state.IsValid()))
        return false;
...
DBErrors CWalletDB::LoadWallet(CWallet *pwallet) {
  ...
  for (CAccountingEntry &entry : pwallet->laccentries) {
    pwallet->wtxOrdered.insert(
        make_pair(entry.nOrderPos, CWallet::TxPair(nullptr, &entry)));
  }
...
DBErrors CWalletDB::ZapSelectTx(std::vector<uint256> &vTxHashIn,
  ...
  for (uint256 hash : vTxHash) {
    ...
    if (it == vTxHashIn.end()) {
      ...
      if (!EraseTx(hash)) {
        LogPrint(BCLog::DB,
                 "Transaction was found for deletion but returned database "
                 "error: %s\n",
                 hash.GetHex());
        delerror = true;
      }
