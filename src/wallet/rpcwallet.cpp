...
bool EnsureWalletIsAvailable(CWallet *const pwallet, bool avoidException) {
  ...
  if (::vpwallets.empty()) {
    throw JSONRPCError(RPC_METHOD_NOT_FOUND,
                       "Method not found (wallet method is disabled because no "
                       "wallet is loaded)");
  }
...
void EnsureWalletIsUnlocked(CWallet *const pwallet) {
  ...
  if (fWalletUnlockWithoutTransactions) {
    throw JSONRPCError(RPC_WALLET_UNLOCK_NEEDED,
                       "Error: Wallet is unlocked for Hive Mining and Rialto "
                       "Messaging only, please fully unlock it first.");
  }
}
...
UniValue getaccountaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nDEPRECATED. Returns the current LitecoinCash address for receiving "
...
UniValue getrawchangeaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address_type\"           (string, optional) The address type to "
        "use. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\". Default "
  ...
  if (!request.params[0].isNull()) {
    ...
    if (output_type == OUTPUT_TYPE_NONE) {
      throw JSONRPCError(
          RPC_INVALID_ADDRESS_OR_KEY,
          strprintf("Unknown address type '%s'", request.params[0].get_str()));
    }
  }
...
UniValue getaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"address\"         (string, required) The litecoincash address "
        "for account lookup.\n"
        "\nResult:\n"
...
UniValue getaddressesbyaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "  \"address\"         (string) a litecoincash address associated with "
        "the given account\n"
        "  ,...\n"
        "]\n"
        "\nExamples:\n" +
...
UniValue sendtoaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
    throw std::runtime_error(
        ...
        "litecoincash than you enter in the amount field.\n"
        ...
        HelpExampleCli("sendtoaddress",
                       "\"LEr4HnaefWYHbMGXcFp2Po1NPRUeIk8km2\" 0.1") +
        HelpExampleCli("sendtoaddress", "\"LEr4HnaefWYHbMGXcFp2Po1NPRUeIk8km2\""
                                        " 0.1 \"donation\" \"seans outpost\"") +
        HelpExampleCli(
            "sendtoaddress",
            "\"LEr4HnaefWYHbMGXcFp2Po1NPRUeIk8km2\" 0.1 \"\" \"\" true") +
...
UniValue getbeecost(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "Defaults to current tip height.\n"
        "\nResult:\n"
        "amount                    (numeric) The amount in " +
        CURRENCY_UNIT +
        "\n"
        "\nExamples:\n" +
        HelpExampleCli("getbeecost", "") + HelpExampleRpc("getbeecost", ""));
  ...
  int height = !request.params[0].isNull() ? request.params[0].get_int()
                                           : chainActive.Height();
...
UniValue createbees(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 4)
    throw std::runtime_error(
        ...
        "\"change_address\" )\n"
        "\nCreate one or more bees in a single transaction, sign it, and "
        "broadcast it to the network.\n" +
        HelpRequiringPassphrase(pwallet) +
        "\nArguments:\n"
        "1. bee_count              (numeric, required) The number of bees to "
        "create.\n"
        "2. community_contrib      (boolean, optional, default=true) If true, "
        "a small percentage of bee creation cost will be paid to a community "
        "fund.\n"
        "3. \"honey_address\"        (string, optional) The " +
        CURRENCY_UNIT +
        " address to receive rewards for blocks mined by bee(s) created in "
        "this transaction.\n"
        "4. \"change_address\"       (string, optional, default pool address) "
        "The " +
        CURRENCY_UNIT +
        ...
        HelpExampleCli("createbees",
                       "5 true \"Cfkv9pniUJ2UoWvaukgD5Ksqx5EsVLzsCk\"") +
        HelpExampleRpc("createbees", "12") +
        HelpExampleRpc("createbees",
                       "34 false \"Cfkv9pniUJ2UoWvaukgD5Ksqx5EsVLzsCk\""));
  ...
  if (pwallet->CreateBeeTransaction(
          beeCount, wtxNew, reservekeyChange, reservekeyHoney, honeyAddress,
          changeAddress, communityContrib, strError, Params().GetConsensus())) {
    CValidationState state;
    if (honeyAddress.empty())

      reservekeyHoney.KeepKey();
    if (!pwallet->CommitTransaction(wtxNew, reservekeyChange, g_connman.get(),
                                    state))
      throw JSONRPCError(
          RPC_WALLET_BCT_FAIL,
          "Error: Bee creation transaction was rejected. Reason given: " +
              state.GetRejectReason());
...
UniValue rialtoregisternick(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
    throw std::runtime_error(
        "rialtoregisternick \"nickname\" ( \"nick_address\", "
        "\"change_address\" )\n"
        "\nRegister a nickname and associated public key in the Rialto White "
        "Pages.\n"
        "\n"
        "Nickname registration fees are:\n"
        "\n"
        "5-20 characters: " +
        ValueFromAmount(consensusParams.nickCreationCostStandard).getValStr() +
        " " + CURRENCY_UNIT +
        "\n"
        "4 characters: " +
        ValueFromAmount(consensusParams.nickCreationCost4Char).getValStr() +
        " " + CURRENCY_UNIT +
        "\n"
        "3 characters: " +
        ValueFromAmount(consensusParams.nickCreationCost3Char).getValStr() +
        " " + CURRENCY_UNIT + "\n" + HelpRequiringPassphrase(pwallet) +
        "\nArguments:\n"
        "1. \"nickname\"             (string, required) The nickname to "
        "register. Must be 3-20 characters, and only lower case letters and "
        "underscores.\n"
        "2. \"nick_address\"         (string, optional) " +
        CURRENCY_UNIT +
        " address to associate with the nickname. If not provided, a new "
        "address will be generated.\n"
        "3. \"change_address\"       (string, optional, default pool address) "
        "The " +
        CURRENCY_UNIT +
        " address to receive the change from the registration transaction.\n"
        "\nResult:\n"
        "\"txid\"                    (string) The transaction id of the nick "
        "registration transaction.\n"
        "\nExamples:\n" +
        HelpExampleCli("rialtoregisternick", "\"b0ssman\"") +
        HelpExampleCli("rialtoregisternick",
                       "\"b0ssman\" \"Cfkv9pniUJ2UoWvaukgD5Ksqx5EsVLzsCk\"") +
        HelpExampleRpc("rialtoregisternick", "\"b0ssman\"") +
        HelpExampleRpc("rialtoregisternick",
                       "\"b0ssman\" \"Cfkv9pniUJ2UoWvaukgD5Ksqx5EsVLzsCk\""));
  ...
  if (pwallet->CreateNickRegistrationTransaction(
          nickname, wtxNew, reservekeyChange, reservekeyNickAddress,
          nickAddress, changeAddress, strError, Params().GetConsensus())) {
    CValidationState state;
    ...
    if (!pwallet->CommitTransaction(wtxNew, reservekeyChange, g_connman.get(),
      ...
      throw JSONRPCError(
          RPC_WALLET_NCT_FAIL,
          "Error: Nick creation transaction was rejected. Reason given: " +
              state.GetRejectReason());
...
UniValue rialtoisnickregistered(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nCheck if a given nickname is registered in the Rialto White "
        "Pages.\n" +
        HelpRequiringPassphrase(pwallet) +
        "\nArguments:\n"
        "1. \"nickname\"             (string, required) The nickname to look "
        "up.\n"
        "\nResult:\n"
        "true|false                  (boolean) true if the nickname is "
        "registered.\n"
        "\nExamples:\n" +
  ...
  if (!IsRialtoEnabled(pindexPrev, consensusParams))
    throw JSONRPCError(RPC_RIALTO_ERROR,
                       "Error: Rialto is not yet enabled on the network");
...
UniValue rialtorebuildwhitepages(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 0) {
    throw std::runtime_error("rialtorebuildwhitepages\n"
                             "\nReplay the local blockchain from the first "
                             "Rialto block, to catch missing registrations.\n"
                             "\nShould only be required if you have upgraded "
                             "from 0.16.3 after Rialto activation.\n"
                             "\nResult:\n"
                             "\"ok\"                      (string) "
                             "Confirmation the rebuild has finished.\n"
                             "\nExamples:\n" +
  ...
  int activationHeight = VersionBitsTipStateSinceHeight(
      consensusParams, Consensus::DEPLOYMENT_RIALTO);
...
UniValue rialtoblocknick(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nArguments:\n"
        "1. \"nickname\"             (string, required) The destination "
        "nickname.\n"
        "\nResult:\n"
  ...
  if (RialtoNickIsLocal(nickname))
    throw JSONRPCError(RPC_RIALTO_ERROR,
                       "Error: Can't block one of your own nicks.");
...
UniValue rialtounblocknick(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "\nUnblock a Rialto nick. Messages from that nick will no longer be "
        "ignored.\n"
        "\nArguments:\n"
        "1. \"nickname\"             (string, required) The destination "
        "nickname.\n"
...
UniValue rialtoisnickblocked(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "nickname.\n"
        "\nResult:\n"
        "true|false                  (boolean) true if nick is blocked, false "
        "if not.\n"
        "\nExamples:\n" +
...
UniValue rialtogetincomingmessages(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "[\n"
        "  [\n"
        "    timestamp,          (numeric) The amount in " +
        CURRENCY_UNIT +
        "\n"
        "    \"from_nick\",      (string) The sender's Rialto nickname\n"
        "    \"to_nick\",        (string) The recipient's Rialto nickname "
        "(you)\n"
...
UniValue rialtoencrypt(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 3)
    throw std::runtime_error(
        ...
        "3. \"message\"              (string, required) The message to "
        "encrypt.\n"
        "\nResult:\n"
        "\"timestamp\"               (numeric) Sent timestamp as encoded in "
        "Rialto envelopes.\n"
        "\nExamples:\n" +
  ...
  if ((g_connman->GetLocalServices() & NODE_RIALTO) != NODE_RIALTO)
    throw JSONRPCError(RPC_RIALTO_ERROR,
                       "Error: Rialto relay is not enabled on this node.");
  ...
  if (!rialtoPeers)
    throw JSONRPCError(RPC_RIALTO_ERROR,
                       "Error: No connected peers support Rialto relaying.");

  if (pwallet->IsLocked())
    throw JSONRPCError(
        RPC_RIALTO_ERROR,
        "Error: Can't encrypt messages with locked wallet; unlock it first.");
  ...
  if (nickTo == "")
    throw JSONRPCError(RPC_RIALTO_ERROR,
                       "Error: No destination nick provided.");

  if (!RialtoNickExists(nickTo))
    throw JSONRPCError(RPC_RIALTO_ERROR,
                       "Error: Recipient's nickname is not registered.");
  ...
  std::string err;
  uint32_t timestamp;
  if (!RialtoEncryptMessage(nickFrom, nickTo, message, ciphertext, timestamp,
                            err))
...
UniValue getnetworkhiveinfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "\nLists the mature and immature bee populations across the entire "
        "network. Optionally returns values to plot the future bee "
        "population.\n"
        "\nArguments:\n"
        "1. include_graph                  (boolean, optional, default=false) "
        "Include bee population graph\n"
        "\nResult:\n"
        "{\n"
        "  immature_bee_count,             (numeric) The number of immature "
        "bees\n"
        "  immature_bct_count,             (numeric) The number of immature "
        "Bee Creation Transactions\n"
        "  mature_bee_count,               (numeric) The number of mature "
        "bees\n"
        "  mature_bct_count,               (numeric) The number of mature Bee "
        "Creation Transactions\n"
        "  honey_pot,                      (numeric) Total potential network "
        "rewards available during bee lifespan (in " +
        CURRENCY_UNIT +
        ")\n"
        "  mature_bee_pop_graph: [ ... ],  (numeric array) Graph points for "
        "mature bee population over upcoming bee gestation span + bee lifespan "
        "blocks\n"
        "  immature_bee_pop_graph: [ ... ] (numeric array) Graph points for "
        "immature bee population over upcoming bee gestation span\n"
        "}\n"
        "\nExamples:\n" +
  ...
  int globalImmatureBees, globalImmatureBCTs, globalMatureBees,
      globalMatureBCTs;
  CAmount potentialRewards;
  if (!GetNetworkHiveInfo(globalImmatureBees, globalImmatureBCTs,
                          globalMatureBees, globalMatureBCTs, potentialRewards,
                          consensusParams, includeGraph))
    throw std::runtime_error(
        "Error: A block required to calculate network bee population was not "
        "available (pruned data / not found on disk)");
...
UniValue getbeecreationtxid(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error("getbeecreationtxid \"honey_txid\"\n"
                             "\nGives the BCT transaction id for a given honey "
                             "transaction received by this wallet.\n"
                             "\nArguments:\n"
                             "1. honey_txid          (string) Transaction ID "
                             "of a honey transaction received by this wallet\n"
                             "\nResult:\n"
                             "\"hex\"                  (string) Transaction ID "
                             "of the BCT responsible for this honey\n"
                             "\nExamples:\n" +
                             HelpExampleCli("getbeecreationtxid",
                                            "bd76be6d12dfd072a605fd85a2aa956f6f"
                                            "5e0dee5dbbb0b4b5da5e72966b9dfd"));
  ...
  if (wtx.tx->vout[0].scriptPubKey.size() < 144)
    throw JSONRPCError(RPC_INVALID_PARAMETER, "Malformed honey transaction!");

  std::vector<unsigned char> bctTxId(&wtx.tx->vout[0].scriptPubKey[14],
                                     &wtx.tx->vout[0].scriptPubKey[14 + 64]);
...
UniValue getbctinfo(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "\nLists the status of a single bee creation transaction from your "
        "current hive.\n"
        "\nArguments:\n"
        "1. bct_txid               (string) Transaction ID of a bee creation "
        "transaction made by this wallet\n"
        "2. min_honey_confirms     (numeric, optional, default=1) Only include "
        "honey rewards with at least this many confirmations\n"
        "\nResult:\n"
        "{\n"
        "    \"txid\",               (string) Transaction ID of the bee "
        "creation transaction\n"
        "    time,                 (numeric) Timestamp of block containing the "
        "bee creation transaction (only present once tx is in a block)\n"
        "    bee_count,            (numeric) The number of bees created\n"
        "    community_contrib,    (boolean) If true, indicates that a portion "
        "of the bee creation fee was paid to the community fund\n"
        "    \"bee_status\",         (string) immature | mature | dead. Only "
        "mature bees are capable of mining\n"
        "    \"honey_address\",      (string) The address which will receive "
        "block rewards for blocks minted by the bees\n"
        "    bee_fee_paid,         (numeric) Total bee creation fee (in " +
        CURRENCY_UNIT +
        ")\n"
        "    rewards_paid,         (numeric) The amount of block rewards "
        "earned by the bees (in " +
        CURRENCY_UNIT +
        ")\n"
        "    profit,               (numeric) The profit earned by the bees "
        "(in " +
        CURRENCY_UNIT +
        ")\n"
        "    blocks_found,         (numeric) The number of blocks minted by "
        "the bees\n"
        "    blocks_left           (numeric) The number of blocks of bee "
        "lifespan remaining\n"
        "}\n"
        "\nExamples:\n" +
        HelpExampleCli("getbctinfo", "1f14fc54cfce1ec53f5a8d36d8aef7ae8485e4cd6"
                                     "93b6f404c5d2504017ecd59") +
        HelpExampleCli("getbctinfo", "1f14fc54cfce1ec53f5a8d36d8aef7ae8485e4cd6"
                                     "93b6f404c5d2504017ecd59, 10"));
  ...
  CBeeCreationTransactionInfo bct =
      pwallet->GetBCT(wtx, true, true, consensusParams, minHoneyConfirms);
  UniValue jsonResults(UniValue::VOBJ);
  if (bct.txid != "") {
    ...
    jsonResults.push_back(
        Pair("bee_fee_paid", ValueFromAmount(bct.beeFeePaid)));
    jsonResults.push_back(
        Pair("rewards_paid", ValueFromAmount(bct.rewardsPaid)));
...
UniValue gethiveinfo(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "\nLists the status of your current hive, broken down by bee creation "
        "transaction.\n"
        "\nArguments:\n"
        "1. include_dead           (boolean, optional, default=false) Include "
        "bees whose lifespans have expired\n"
        "2. min_honey_confirms     (numeric, optional, default=1) Only include "
        "honey rewards with at least this many confirmations\n"
        "\nResult:\n"
        "{\n"
        ...
        "        bee_fee_paid,             (numeric) Total bee creation fees "
        "(in " +
        CURRENCY_UNIT +
        ")\n"
        "        rewards_paid,             (numeric) Total rewards paid (in " +
        CURRENCY_UNIT +
        ")\n"
        "        profit,                   (numeric) Total rewards paid (in " +
        CURRENCY_UNIT +
        ")\n"
        "        \"warnings\"                (string) Warnings; empty if no "
        "warnings\n"
        "    },\n"
        "    bees: [\n"
        "        {\n"
        "            \"txid\",               (string) Transaction ID of the "
        "bee creation transaction\n"
        "            time,                 (numeric) Timestamp of block "
        "containing the bee creation transaction (only present once tx is in a "
        "block)\n"
        "            bee_count,            (numeric) The number of bees "
        "created\n"
        "            community_contrib,    (boolean) If true, indicates that a "
        "portion of the bee creation fee was paid to the community fund\n"
        "            \"bee_status\",         (string) immature | mature | "
        "dead. Only mature bees are capable of mining\n"
        "            \"honey_address\",      (string) The address which will "
        "receive block rewards for blocks minted by the bees\n"
        "            bee_fee_paid,         (numeric) Total bee creation fee "
        "(in " +
        CURRENCY_UNIT +
        ")\n"
        "            rewards_paid,         (numeric) The amount of block "
        "rewards earned by the bees (in " +
        CURRENCY_UNIT +
        ")\n"
        "            profit,               (numeric) The profit earned by the "
        "bees (in " +
        CURRENCY_UNIT +
        ")\n"
        "            blocks_found,         (numeric) The number of blocks "
        "minted by the bees\n"
        "            blocks_left           (numeric) The number of blocks of "
        "bee lifespan remaining\n"
        "        },...\n"
        "    ]\n"
        "}\n"
        "\nExamples:\n" +
  ...
  std::vector<CBeeCreationTransactionInfo> bcts =
      pwallet->GetBCTs(includeDead, true, consensusParams, minHoneyConfirms);
  ...
  summary.push_back(
      Pair("profit", ValueFromAmount(totalRewards - totalBeeFee)));
  summary.push_back(
      Pair("warnings", pwallet->IsLocked()
                           ? "Wallet is locked and must be unlocked to mine"
                           : ""));
...
UniValue listaddressgroupings(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        CURRENCY_UNIT +
        "\n"
        "      \"account\"             (string, optional) DEPRECATED. The "
        "account\n"
        "    ]\n"
        "    ,...\n"
        "  ]\n"
...
UniValue signmessage(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 2)
    throw std::runtime_error(
        ...
        HelpExampleCli(
            "signmessage",
            "\"LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM2\" \"my message\"") +
        "\nVerify the signature\n" +
        HelpExampleCli("verifymessage", "\"LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM2\""
...
UniValue getreceivedbyaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "confirmations\n" +
        HelpExampleCli("getreceivedbyaddress",
                       "\"LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM2\" 0") +
        "\nThe amount with at least 6 confirmations\n" +
        HelpExampleCli("getreceivedbyaddress",
                       "\"LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM2\" 6") +
        "\nAs a json rpc call\n" +
...
UniValue getreceivedbyaccount(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "\nExamples:\n"
        "\nAmount received by the default account with at least 1 "
        "confirmation\n" +
...
UniValue getbalance(const JSONRPCRequest &request) {
  ...
  if (account_value.isNull()) {
    ...
    if (!include_watchonly.isNull()) {
      throw JSONRPCError(RPC_INVALID_PARAMETER,
                         "getbalance include_watchonly option is only "
                         "currently supported if an account is specified");
    }
...
UniValue movecmd(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 3 || request.params.size() > 5)
    throw std::runtime_error(
        ...
        "5. \"comment\"       (string, optional) An optional comment, stored "
        "in the wallet only.\n"
        "\nResult:\n"
        "true|false           (boolean) true if successful.\n"
        "\nExamples:\n"
...
UniValue sendfrom(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 3 || request.params.size() > 6)
    throw std::runtime_error(
        ...
        "                       the spend.\n"
        "2. \"toaddress\"         (string, required) The litecoincash address "
        "to send funds to.\n"
        ...
        HelpExampleCli("sendfrom",
                       "\"\" \"LEr4HnaefWYHbMGXcFp2Po1NPRUeIk8km2\" 0.01") +
        "\nSend 0.01 from the tabby account to the given address, funds must "
        "have at least 6 confirmations\n" +
        HelpExampleCli("sendfrom",
                       "\"tabby\" \"LEr4HnaefWYHbMGXcFp2Po1NPRUeIk8km2\" 0.01 "
                       "6 \"donation\" \"seans outpost\"") +
        "\nAs a json rpc call\n" +
        HelpExampleRpc("sendfrom",
                       "\"tabby\", \"LEr4HnaefWYHbMGXcFp2Po1NPRUeIk8km2\", "
...
UniValue sendmany(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 8)
    throw std::runtime_error(
        ...
        "    {\n"
        "      \"address\":amount   (numeric or string) The litecoincash "
        ...
        "litecoincash than you enter in their corresponding amount field.\n"
        ...
        HelpExampleCli("sendmany",
                       "\"\" "
                       "\"{\\\"LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM2\\\":0.01,"
                       "\\\"LbhhnrHHVFP1eUjP1tdNIYeEVsNHfN9FCw\\\":0.02}\"") +
        "\nSend two amounts to two different addresses setting the "
        "confirmation and comment:\n" +
        HelpExampleCli("sendmany",
                       "\"\" "
                       "\"{\\\"LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM2\\\":0.01,"
                       "\\\"LbhhnrHHVFP1eUjP1tdNIYeEVsNHfN9FCw\\\":0.02}\" 6 "
                       "\"testing\"") +
        ...
        HelpExampleCli(
            ...
            "\"\" "
            "\"{\\\"LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM2\\\":0.01,"
            "\\\"LbhhnrHHVFP1eUjP1tdNIYeEVsNHfN9FCw\\\":0.02}\" 1 \"\" "
            "\"[\\\"LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM2\\\","
        ...
        HelpExampleRpc(
            ...
            "\"\", "
            "{\"LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM2\":0.01,"
            "\"LbhhnrHHVFP1eUjP1tdNIYeEVsNHfN9FCw\":0.02}, 6, \"testing\""));
  ...
  for (const std::string &name_ : keys) {
    ...
    if (!IsValidDestination(dest)) {
      throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                         std::string("Invalid LitecoinCash address: ") + name_);
    }
...
UniValue addmultisigaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 4) {
    std::string msg =
        ...
        "\nArguments:\n"
        "1. nrequired                      (numeric, required) The number of "
        "required signatures out of the n keys or addresses.\n"
        "2. \"keys\"                         (string, required) A json array "
        "of litecoincash addresses or hex-encoded public keys\n"
        "     [\n"
        "       \"address\"                  (string) litecoincash address or "
        "hex-encoded public key\n"
        "       ...,\n"
        "     ]\n"
        "3. \"account\"                      (string, optional) DEPRECATED. An "
        "account to assign the addresses to.\n"
        "4. \"address_type\"                 (string, optional) The address "
        "type to use. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\". "
        ...
        "}\n"
        "\nResult (DEPRECATED. To see this result in v0.16 instead, please "
        "start litecoincashd with -deprecatedrpc=addmultisigaddress).\n"
        "        clients should transition to the new output api before "
        "upgrading to v0.17.\n"
        "\"address\"                         (string) A litecoincash address "
  ...
  if (!request.params[3].isNull()) {
    ...
    if (output_type == OUTPUT_TYPE_NONE) {
      throw JSONRPCError(
          RPC_INVALID_ADDRESS_OR_KEY,
          strprintf("Unknown address type '%s'", request.params[3].get_str()));
    }
  }
...
public:
  ...
  explicit Witnessifier(CWallet *_pwallet)
      : pwallet(_pwallet), already_witness(false) {}
  ...
  template <typename T> bool operator()(const T &dest) { return false; }
};
...
UniValue addwitnessaddress(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
    std::string msg =
        ...
        "\nDEPRECATED: set the address_type argument of getnewaddress, or "
        "option -addresstype=[bech32|p2sh-segwit] instead.\n"
        "Add a witness address for a script (with pubkey or redeemscript "
        "known). Requires a new wallet backup.\n"
        "It returns the witness script.\n"

        "\nArguments:\n"
        "1. \"address\"       (string, required) An address known to the "
        "wallet\n"
        "2. p2sh            (bool, optional, default=true) Embed inside P2SH\n"

        "\nResult:\n"
        "\"witnessaddress\",  (string) The value of the new address (P2SH or "
        "BIP173).\n"
        "}\n";
    throw std::runtime_error(msg);
  }
  ...
  if (!IsDeprecatedRPCEnabled("addwitnessaddress")) {
    throw JSONRPCError(
        ...
        "addwitnessaddress is deprecated and will be fully removed in v0.17. "
        "To use addwitnessaddress in v0.16, restart litecoincashd with "
        "-deprecatedrpc=addwitnessaddress.\n"
        "Projects should transition to using the address_type argument of "
        "getnewaddress, or option -addresstype=[bech32|p2sh-segwit] "
        "instead.\n");
  }
  ...
  if (!ret) {
    throw JSONRPCError(RPC_WALLET_ERROR,
                       "Public key or redeemscript not known to wallet, or the "
                       "key is uncompressed");
  }
...
UniValue listtransactions(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 4)
    throw std::runtime_error(
        ...
        "                                                It will be \"\" for "
        "the default account.\n"
        "    \"address\":\"address\",    (string) The litecoincash address of "
        "the transaction. Not present for \n"
        "                                                move transactions "
        "(category = move).\n"
        "    \"ishoney\": xxx,           (bool) Whether this transaction is "
        "honey (hive block coinbase transaction)\n"
...
UniValue listsinceblock(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 4)
    throw std::runtime_error(
        ...
        HelpExampleCli("listsinceblock", "\"2c5a0ff9e4d8a7cdece6cc0f11d8f949a0c"
                                         "58b2028fab79b90485a811253e217\" 6") +
        HelpExampleRpc("listsinceblock", "\"2c5a0ff9e4d8a7cdece6cc0f11d8f949a0c"
                                         "58b2028fab79b90485a811253e217\", 6"));
...
UniValue gettransaction(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "account.\n"
        "      \"address\" : \"address\",          (string) The litecoincash "
        ...
        "\nExamples:\n" +
        HelpExampleCli("gettransaction", "\"c1700d6dd3e690866de56686e893cbe4e63"
                                         "7eb5d84e3591cdfbbdbb0fcee49f8\"") +
        HelpExampleCli("gettransaction",
                       "\"c1700d6dd3e690866de56686e893cbe4e637eb5d84e3591cdfbbd"
                       "bb0fcee49f8\" true") +
        HelpExampleRpc("gettransaction", "\"c1700d6dd3e690866de56686e893cbe4e63"
                                         "7eb5d84e3591cdfbbdbb0fcee49f8\""));
...
static void SetHiveOnly() { fWalletUnlockWithoutTransactions = true; }
...
UniValue walletpassphrase(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 2) {
    throw std::runtime_error(
        ...
        "keys such as sending litecoincash\n"
        "\nArguments:\n"
        "1. \"passphrase\"       (string, required) The wallet passphrase\n"
        "2. timeout            (numeric, required) The time to keep the "
        "decryption key in seconds. Limited to at most 1073741824 (2^30) "
        "seconds.\n"
        "                                          Any value greater than "
        "1073741824 seconds will be set to 1073741824 seconds.\n"
        "\nNote:\n"
  ...
  if (!wasUnLockedHiveOnly) {
    ...
    RPCRunLater(strprintf("sethiveonly(%s)", pwallet->GetName()),
                boost::bind(SetHiveOnly), nSleepTime);
  }
...
UniValue walletpassphrase_hive_rialto(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 1) {
    throw std::runtime_error(
        ...
        "\nStores the wallet decryption key in memory indefinitely for hive "
        "mining and Rialto use only.\n"
        "This is needed to enable the hive mining thread to run, and to "
        "decrypt Rialto messages.\n"
        "\n"
        "Performing other transactions related to private keys, such as "
        "sending " +
        CURRENCY_UNIT +
        ", is not enabled\n"
  ...
  if (!pwallet->IsCrypted()) {
    throw JSONRPCError(RPC_WALLET_WRONG_ENC_STATE,
                       "Error: running with an unencrypted wallet, but "
                       "walletpassphrase_hive_rialto was called.");
  }
...
UniValue walletcandecryptrialto(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 0) {
    throw std::runtime_error(
        ...
        "\nResult:\n"
        "true|false                  (boolean) Whether the command was "
        "successful or not\n"
...
UniValue walletlock(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 0) {
    throw std::runtime_error(
        ...
        HelpExampleCli("sendtoaddress",
                       "\"LEr4HnaefWYHbMGXcFp2Po1NPRUeIk8km2\" 1.0") +
...
UniValue encryptwallet(const JSONRPCRequest &request) {
  ...
  StartShutdown();
  return "wallet encrypted; LitecoinCash server stopping, restart to run with "
...
UniValue lockunspent(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "selection, when spending litecoincash.\n"
        ...
        HelpExampleCli("lockunspent",
                       "false "
                       "\"[{\\\"txid\\\":"
                       "\\\"daaf44ec1e64e66ffda4a7e195ea60d5c8c8b602fc6bf229285"
                       "82d3ff2530462\\\",\\\"vout\\\":1}]\"") +
...
UniValue getwalletinfo(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        CURRENCY_UNIT +
        "\n"
        "  \"unconfirmed_balance\": xxx,      (numeric) the total unconfirmed "
        "balance of the wallet in " +
        CURRENCY_UNIT +
        "\n"
        ...
        CURRENCY_UNIT +
        "/kB\n"
        "  \"hdmasterkeyid\": \"<hash160>\"     (string, optional) the Hash160 "
        "of the HD master pubkey (only present when HD is enabled)\n"
        "}\n"
        "\nExamples:\n" +
...
UniValue resendwallettransactions(const JSONRPCRequest &request) {
  ...
  if (!pwallet->GetBroadcastTransactions()) {
    throw JSONRPCError(RPC_WALLET_ERROR,
                       "Error: Wallet transaction broadcasting is disabled "
...
UniValue listunspent(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 5)
    throw std::runtime_error(
        ...
        "confirmations to filter\n"
        "3. \"addresses\"      (string) A json array of litecoincash addresses "
        "to filter\n"
        "    [\n"
        ...
        HelpExampleCli("listunspent",
                       "6 9999999 "
                       "\"[\\\"LGPYcOdyoBnraaWX5tknkJZZWafjRAGVzx\\\","
                       "\\\"LLmraTr3qBjE2YseA3CnZ55la4TQmWnRY3\\\"]\"") +
  ...
  if (!request.params[2].isNull()) {
    ...
    for (unsigned int idx = 0; idx < inputs.size(); idx++) {
      ...
      if (!IsValidDestination(dest)) {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                           std::string("Invalid LitecoinCash address: ") +
                               input.get_str());
      }
...
UniValue fundrawtransaction(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
    throw std::runtime_error(
        ...
        "address) The litecoincash address to receive the change\n"
        "     \"changePosition\"         (numeric, optional, default random) "
        "The index of the change output\n"
        "     \"change_type\"            (string, optional) The output type to "
        "use. Only valid if changeAddress is not specified. Options are "
        "\"legacy\", \"p2sh-segwit\", and \"bech32\". Default is set by "
        "-changetype.\n"
        ...
        "makes wallet determine the fee) Set a specific fee rate in " +
        CURRENCY_UNIT +
        "/kB\n"
        ...
        "3. iswitness               (boolean, optional) Whether the "
        "transaction hex is a serialized witness transaction \n"
        "                              If iswitness is not present, heuristic "
        "tests will be used in decoding\n"
  ...
  if (!request.params[1].isNull()) {
    if (request.params[1].type() == UniValue::VBOOL) {
      ...
      RPCTypeCheck(request.params,
                   {UniValue::VSTR, UniValue::VOBJ, UniValue::VBOOL});
      ...
      if (options.exists("changeAddress")) {
        ...
        if (!IsValidDestination(dest)) {
          throw JSONRPCError(
              RPC_INVALID_ADDRESS_OR_KEY,
              "changeAddress must be a valid litecoincash address");
        }
      ...
      if (options.exists("change_type")) {
        if (options.exists("changeAddress")) {
          throw JSONRPCError(
              RPC_INVALID_PARAMETER,
              "Cannot specify both changeAddress and address_type options");
        }
        coinControl.change_type = ParseOutputType(
            options["change_type"].get_str(), coinControl.change_type);
        if (coinControl.change_type == OUTPUT_TYPE_NONE) {
          throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
                             strprintf("Unknown change type '%s'",
                                       options["change_type"].get_str()));
        }
      }
  ...
  bool try_witness =
      request.params[2].isNull() ? true : request.params[2].get_bool();
  bool try_no_witness =
      request.params[2].isNull() ? true : !request.params[2].get_bool();
...
UniValue bumpfee(const JSONRPCRequest &request) {
  ...
  if (!request.params[1].isNull()) {
    ...
    if (options.exists("confTarget") && options.exists("totalFee")) {
      throw JSONRPCError(
          RPC_INVALID_PARAMETER,
          "confTarget and totalFee options should not both be set. Please "
          "provide either a confirmation target for fee estimation or an "
          "explicit total fee for the transaction.");
  ...
  feebumper::Result res = feebumper::CreateTransaction(
      pwallet, hash, coin_control, totalFee, errors, old_fee, new_fee, mtx);
  ...
  uint256 txid;
  if (feebumper::CommitTransaction(pwallet, hash, std::move(mtx), errors,
                                   txid) != feebumper::Result::OK) {
    throw JSONRPCError(RPC_WALLET_ERROR, errors[0]);
  }
  UniValue result(UniValue::VOBJ);
...
UniValue rescanblockchain(const JSONRPCRequest &request) {
  ...
  if (request.fHelp || request.params.size() > 2) {
    throw std::runtime_error(
        ...
        "rescan should start\n"
        "2. \"stop_height\"     (numeric, optional) the last block height that "
        "should be scanned\n"
        "\nResult:\n"
        "{\n"
        "  \"start_height\"     (numeric) The block height where the rescan "
        "has started. If omitted, rescan started from the genesis block.\n"
  ...
  if (fPruneMode) {
    ...
    while (block && block->nHeight >= pindexStart->nHeight) {
      if (!(block->nStatus & BLOCK_HAVE_DATA)) {
        throw JSONRPCError(
            RPC_MISC_ERROR,
            "Can't rescan beyond pruned data. Use RPC call getblockchaininfo "
            "to determine your pruned height.");
      }
      block = block->pprev;
    }
  }
...
static const CRPCCommand commands[] = {
    {"wallet", "getbeecost", &getbeecost, {"height"}},

    {"wallet",
     "createbees",
     &createbees,
     {"bee_count", "community_contrib", "honey_address"}},

    {"wallet",
     "gethiveinfo",
     &gethiveinfo,
     {"include_dead", "min_honey_confirms"}},

    {"wallet", "getnetworkhiveinfo", &getnetworkhiveinfo, {"include_graph"}},

    {"wallet", "getbeecreationtxid", &getbeecreationtxid, {"honey_txid"}},

    {"wallet", "getbctinfo", &getbctinfo, {"bct_txid", "min_honey_confirms"}},

    {"wallet",
     "rialtoregisternick",
     &rialtoregisternick,
     {"nickname", "nick_address", "change_address"}},

    {"wallet", "rialtoisnickregistered", &rialtoisnickregistered, {"nickname"}},

    {"wallet", "rialtogetmynicks", &rialtogetmynicks, {}},

    {"wallet", "rialtorebuildwhitepages", &rialtorebuildwhitepages, {}},

    {"wallet",
     "rialtoencrypt",
     &rialtoencrypt,
     {"sender_nick", "destination_nick", "message"}},

    {"wallet", "rialtoblocknick", &rialtoblocknick, {"nickname"}},

    {"wallet", "rialtounblocknick", &rialtounblocknick, {"nickname"}},

    {"wallet", "rialtoisnickblocked", &rialtoisnickblocked, {"nickname"}},

    {"wallet", "rialtogetblockednicks", &rialtogetblockednicks, {}},

    {"wallet", "rialtogetincomingmessages", &rialtogetincomingmessages, {}},
    ...
    {"wallet",
     "addmultisigaddress",
     &addmultisigaddress,
     {"nrequired", "keys", "account", "address_type"}},
    ...
    {"wallet", "walletcandecryptrialto", &walletcandecryptrialto, {}},
