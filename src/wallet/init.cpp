...
std::string GetWalletHelpString(bool showDebug) {
  ...
  strUsage += HelpMessageOpt(
      "-addresstype",
      strprintf("What type of addresses to use (\"legacy\", \"p2sh-segwit\", "
                "or \"bech32\", default: \"%s\")",
                FormatOutputType(OUTPUT_TYPE_DEFAULT)));
  strUsage += HelpMessageOpt(
      "-changetype", "What type of change to use (\"legacy\", \"p2sh-segwit\", "
                     "or \"bech32\"). Default is same as -addresstype, except "
                     "when -addresstype=p2sh-segwit a native segwit output is "
  ...
  strUsage += HelpMessageOpt(
      "-discardfee=<amt>",
      strprintf(
          _("The fee rate (in %s/kB) that indicates your tolerance for "
            "discarding change by adding it to the fee (default: %s). "
            "Note: An output is discarded if it is dust at this rate, but we "
            "will always discard up to the dust relay fee and a discard fee "
            "above that is limited by the fee estimate for the longest target"),
  ...
  strUsage += HelpMessageOpt(
      "-txconfirmtarget=<n>",
      strprintf(
          _("If paytxfee is not set, include enough fee so transactions begin "
            "confirmation on average within n blocks (default: %u)"),
          DEFAULT_TX_CONFIRM_TARGET));
  strUsage += HelpMessageOpt(
      "-walletrbf", strprintf(_("Send transactions with full-RBF opt-in "
                                "enabled (RPC only, default: %u)"),
                              DEFAULT_WALLET_RBF));
  ...
  strUsage += HelpMessageOpt(
      ...
      _("Delete all wallet transactions and only recover those parts of the "
        "blockchain through -rescan on startup") +
          " " +
          _("(1 = keep tx meta data e.g. account owner and payment request "
...
bool WalletParameterInteraction() {
  ...
  if (g_address_type == OUTPUT_TYPE_NONE) {
    return InitError(strprintf("Unknown address type '%s'",
                               gArgs.GetArg("-addresstype", "")));
  }

  g_change_type =
      ParseOutputType(gArgs.GetArg("-changetype", ""), OUTPUT_TYPE_NONE);
  if (g_change_type == OUTPUT_TYPE_NONE &&
      !gArgs.GetArg("-changetype", "").empty()) {
    return InitError(
        strprintf("Unknown change type '%s'", gArgs.GetArg("-changetype", "")));
  }
