...
private:
  ...
  bool ConnectTip(CValidationState &state, const CChainParams &chainparams,
...
std::unique_ptr<CRialtoWhitePagesDB> pwhitepages;

std::unique_ptr<CRialtoWhitePagesDB> pmynicks;

std::unique_ptr<CRialtoWhitePagesDB> pblockednicks;
...
bool CheckFinalTx(const CTransaction &tx, int flags) {
  ...
  const int64_t nBlockHeight = static_cast<int64_t>(chainActive.Height()) + 1;

  int64_t nBlockTime;
  if (flags & LOCKTIME_MEDIAN_TIME_PAST) {
    if (chainActive.Tip() != nullptr) {
      nBlockTime = chainActive.Tip()->GetMedianTimePast();
    } else {
...
void UpdateMempoolForReorg(DisconnectedBlockTransactions &disconnectpool,
  ...
  while (it != disconnectpool.queuedTx.get<insertion_order>().rend()) {
    ...
    if (!fAddToMempool || (*it)->IsCoinBase() ||
        !AcceptToMemoryPool(mempool, stateDummy, *it, nullptr, nullptr, true,
                            0)) {
  ...
  mempool.removeForReorg(pcoinsTip.get(), chainActive.Tip()->nHeight + 1,
...
static bool AcceptToMemoryPoolWorker(
    ...
    const CTransactionRef &ptx, bool *pfMissingInputs, int64_t nAcceptTime,
...

  {
    ...
    CAmount mempoolRejectFee =
        pool.GetMinFee(gArgs.GetArg("-maxmempool", DEFAULT_MAX_MEMPOOL_SIZE) *
                       1000000)
            .GetFee(nSize);
    if (!bypass_limits && mempoolRejectFee > 0 &&
    ...
    if (fReplacementTransaction) {
      ...
      CAmount originalFees = 0;
      for (const auto &it : allConflicting) {
        originalFees += it->GetModifiedFee();
      }

      if (nModifiedFees <= originalFees) {
        return state.DoS(
            ...
            strprintf(
                ...
                CFeeRate(originalFees, nConflictingSize).ToString()));
      }

      CAmount additionalFees = nModifiedFees - originalFees;
      if (additionalFees < ::incrementalRelayFee.GetFee(nSize)) {
        return state.DoS(
            0, false, REJECT_INSUFFICIENTFEE, "insufficient fee", false,
            strprintf("rejecting replacement %s, not enough additional fees to "
                      "relay; %s < %s",
                      hash.ToString(), FormatMoney(additionalFees),
    ...
    for (const CTxMemPool::txiter it : allConflicting) {
      LogPrint(BCLog::MEMPOOL,
               "replacing tx %s with %s for %s LCC additional fees, %d delta "
               "bytes\n",
    ...
    bool validForFeeEstimation = !fReplacementTransaction && !bypass_limits &&
                                 IsCurrentForFeeEstimation() &&
                                 pool.HasNoInputsOf(tx);

    pool.addUnchecked(hash, entry, validForFeeEstimation);
...
static bool
AcceptToMemoryPoolWithTime(const CChainParams &chainparams, CTxMemPool &pool,
  ...
  bool res = AcceptToMemoryPoolWorker(
      chainparams, pool, state, tx, pfMissingInputs, nAcceptTime, plTxnReplaced,
...
bool AcceptToMemoryPool(CTxMemPool &pool, CValidationState &state,
  ...
  return AcceptToMemoryPoolWithTime(chainparams, pool, state, tx,
...
bool ReadBlockFromDisk(CBlock &block, const CDiskBlockPos &pos,
  ...
  if (block.IsHiveMined(consensusParams)) {
    if (!CheckHiveProof(&block, consensusParams))
      return error("ReadBlockFromDisk: Errors in Hive block header at %s",
                   pos.ToString());
  } else {
    if (!CheckProofOfWork(block.GetPoWHash(), block.nBits, consensusParams))
      return error("ReadBlockFromDisk: Errors in PoW block header at %s",
                   pos.ToString());
  }
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params &consensusParams) {
  ...
  if (blocksSinceFork > 0 &&
      blocksSinceFork < consensusParams.slowStartBlocks) {
...
CAmount GetBeeCost(int nHeight, const Consensus::Params &consensusParams) {
  ...
  return beeCost <= consensusParams.minBeeCost ? consensusParams.minBeeCost
                                               : beeCost;
}
...
static void AlertNotify(const std::string &strMessage) {
  ...
  std::string escapedMessage = strMessage;
  boost::replace_all(escapedMessage, "'", "'\\''");

  boost::replace_all(strCmd, "%s", "'" + escapedMessage + "'");
...
static void CheckForkWarningConditions() {
  ...
  if (pindexBestForkTip ||
    ...
    if (pindexBestForkTip && pindexBestForkBase) {
      ...
      LogPrintf(
          "%s: Warning: Found invalid chain at least ~6 blocks longer than our "
...
static void CheckForkWarningConditionsOnNewFork(CBlockIndex *pindexNewForkTip) {
  ...
  if (pfork &&
    ...
    pindexBestForkBase = const_cast<CBlockIndex *>(pfork);
  }
...
int GetSpendHeight(const CCoinsViewCache &inputs) {
  ...
  uint256 bestBlockHash = inputs.GetBestBlock();
  CBlockIndex *pindexPrev = nullptr;

  if (!bestBlockHash.IsNull()) {
    auto it = mapBlockIndex.find(bestBlockHash);
...
bool CChainState::ConnectBlock(const CBlock &block, CValidationState &state,
    ...
    if (it != mapBlockIndex.end()) {
      ...
      if (pindexBestHeader != nullptr) {
        CBlockIndex *bhAncestor =
            pindexBestHeader->GetAncestor(pindex->nHeight);
        bestHeaderMatches = (bhAncestor == pindex) &&
  ...
  CAmount blockReward =
      GetBlockSubsidy(pindex->nHeight, chainparams.GetConsensus());
  ...
  if (pindex->nHeight == chainparams.GetConsensus().lastScryptBlock + 1) {
    ...
    } else if (block.vtx[0]->vout[0].scriptPubKey !=
               chainparams.GetConsensus().premineOutputScript) {
      return state.DoS(
          100,
          error("ConnectBlock(): incorrect pubkey on pm coinbase TX (Got %s, "
                "expected %s)",
  ...
  if (!IsInitialBlockDownload()) {
    ...
    for (int i = 0; i < 100 && pindex != nullptr; i++) {
      ...
      if (IsMinotaurXEnabled(pindex, chainParams.GetConsensus())) {
        if ((pindex->nVersion & (int32_t)0xFF00FFFF) != nExpectedVersion &&
            !pindex->GetBlockHeader().IsHiveMined(chainParams.GetConsensus()))
          ++nUpgraded;
      } else {
...
bool CChainState::DisconnectTip(CValidationState &state,
  ...
  {
    CScript scriptPubKeyNCF = GetScriptForDestination(
        DecodeDestination(chainparams.GetConsensus().nickCreationAddress));

    for (int i = block.vtx.size() - 1; i >= 0; i--) {
      const CTransaction &tx = *(block.vtx[i]);
      std::string nick;
      if (tx.IsNCT(chainparams.GetConsensus(), scriptPubKeyNCF, nullptr,
                   &nick)) {
        if (pwhitepages->NickExists(nick)) {
          pwhitepages->RemoveNick(nick);
          LogPrint(BCLog::RIALTO,
                   "Rialto: Removed nick %s from global white pages "
                   "(disconnected tip)\n",
                   nick);
        }
        if (pmynicks->NickExists(nick)) {
          ...
          LogPrint(BCLog::RIALTO,
                   "Rialto: Removed nick %s from local white pages "
                   "(disconnected tip)\n",
                   nick);
        }
      }
...
public:
  explicit ConnectTrace(CTxMemPool &_pool) : blocksConnected(1), pool(_pool) {
    pool.NotifyEntryRemoved.connect(
        boost::bind(&ConnectTrace::NotifyEntryRemoved, this,
...
bool CChainState::ConnectTip(CValidationState &state,
  ...
  if (IsRialtoEnabled(pindexNew, chainparams.GetConsensus())) {
    ...
    for (unsigned long int i = 0; i < blockConnecting.vtx.size(); i++) {
      ...
      if (tx.IsNCT(chainparams.GetConsensus(), scriptPubKeyNCF, &pubKeyStr,
                   &nickname)) {
        if (pwhitepages->NickExists(nickname)) {
          LogPrint(BCLog::RIALTO,
                   "Rialto: Ignoring duplicate registration for nick %s\n",
                   nickname);
        } else {
          CPubKey pubKey(ParseHex(pubKeyStr));
          CTxDestination rialtoDestination =
              GetDestinationForKey(pubKey, OUTPUT_TYPE_LEGACY);

          pwhitepages->SetPubKeyForNick(nickname, pubKeyStr);
          LogPrint(BCLog::RIALTO,
                   "Rialto: Added nick %s to global whitepages\n", nickname);
          ...
          if (keyID) {
            ...
            if (!EnsureWalletIsAvailable(pwallet, true)) {
              LogPrintf("Rialto: ERROR: Can't check if nick %s is local; "
                        "wallet unavailable\n",
                        nickname);
              continue;
            }
            ...
            if (pwallet->GetKey(*keyID, key)) {
              ...
              LogPrint(BCLog::RIALTO,
                       "Rialto: Added our nick %s to local whitepages\n",
                       nickname);
            } else {
            }
          }
...
bool CChainState::ReceivedBlockTransactions(
    const CBlock &block, CValidationState &state, CBlockIndex *pindexNew,
  ...
  if (pindexNew->pprev == nullptr || pindexNew->pprev->nChainTx) {
    ...
    while (!queue.empty()) {
      if (ShutdownRequested()) {
        return state.Error("Shutdown requested");
      }
...
bool CheckBlock(const CBlock &block, CValidationState &state,
  ...
  if (block.IsHiveMined(consensusParams))
    if (!CheckHiveProof(&block, consensusParams))
      return state.DoS(100, false, REJECT_INVALID, "bad-hive-proof", false,
                       "proof of hive failed");
...
bool IsHiveEnabled(const CBlockIndex *pindexPrev,
                   const Consensus::Params &params) {
  LOCK(cs_main);
  return (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_HIVE,
                           versionbitscache) == THRESHOLD_ACTIVE);
}
...
bool IsHive11Enabled(const CBlockIndex *pindexPrev,
                     const Consensus::Params &params) {
  LOCK(cs_main);
  return (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_HIVE_1_1,
                           versionbitscache) == THRESHOLD_ACTIVE);
}
...
bool IsMinotaurXEnabled(const CBlockIndex *pindexPrev,
                        const Consensus::Params &params) {
  LOCK(cs_main);
  return (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_MINOTAURX,
                           versionbitscache) == THRESHOLD_ACTIVE);
}
...
bool IsRialtoEnabled(const CBlockIndex *pindexPrev,
                     const Consensus::Params &params) {
  LOCK(cs_main);
  return (VersionBitsState(pindexPrev, params, Consensus::DEPLOYMENT_RIALTO,
                           versionbitscache) == THRESHOLD_ACTIVE);
}
...
std::vector<std::string> RialtoGetBlockedNicks() {
  LOCK(cs_main);
  std::vector<std::pair<std::string, std::string>> blockedNickPairs =
      pblockednicks->GetAll();
...
bool GetTxByHashAndHeight(const uint256 txHash, const int nHeight,
  ...
  if (block.vtx.size() > 0 &&
      merkleBlock.txn.ExtractMatches(vMatched, vIndex).GetHex() ==
          block.hashMerkleRoot.GetHex() &&
      vMatched.size() == 1 && vMatched[0].ToString() == txHash.ToString())
    for (const auto &tx : block.vtx)
...
static bool ContextualCheckBlockHeader(const CBlockHeader &block,
  ...
  int64_t max_future_block_time =
      IsMinotaurXEnabled(pindexPrev, consensusParams)
  ...
  if (!IsMinotaurXEnabled(pindexPrev, consensusParams)) {
    ...
    if (blockType >= NUM_BLOCK_TYPES)
      return state.Invalid(
          false, REJECT_INVALID, "bad-blocktype",
...
bool ProcessNewBlockHeaders(const std::vector<CBlockHeader> &headers,
    ...
    first_invalid->SetNull();
  {
    ...
    for (const CBlockHeader &header : headers) {
      ...
      if (!g_chainstate.AcceptBlockHeader(header, state, chainparams,
                                          &pindex)) {
        if (first_invalid)
...
bool ProcessNewBlock(const CChainParams &chainparams,
...

  {
    ...
    if (ret) {
      ret = g_chainstate.AcceptBlock(pblock, state, chainparams, &pindex,
...
static void FindFilesToPrune(std::set<int> &setFilesToPrune,
  ...
  LogPrint(BCLog::PRUNE,
           "Prune: target=%dMiB actual=%dMiB diff=%dMiB max_prune_height=%d "
           "removed %d blk/rev pairs\n",
...
bool CChainState::LoadBlockIndex(const Consensus::Params &consensus_params,
  ...
  if (!blocktree.LoadBlockIndexGuts(
          consensus_params,
          [this](const uint256 &hash) { return this->InsertBlockIndex(hash); }))
    return false;
...
CVerifyDB::~CVerifyDB() { uiInterface.ShowProgress("", 100, false); }
  ...
  if (nCheckLevel >= 4) {
    ...
    while (pindex != chainActive.Tip()) {
      ...
      uiInterface.ShowProgress(
          _("Verifying blocks..."),
          std::max(1, std::min(99, 100 - (int)(((double)(chainActive.Height() -
                                                         pindex->nHeight)) /
...
bool CChainState::ReplayBlocks(const CChainParams &params, CCoinsView *view) {
  ...
  if (mapBlockIndex.count(hashHeads[0]) == 0) {
    LogPrintf(
        "WARNING: ReplayBlocks(): new best block %s not found in block index. "
        "Attempting to recover by rolling back chainstate to previous head.\n",
        hashHeads[0].ToString());

    if (hashHeads[1].IsNull() || mapBlockIndex.count(hashHeads[1]) == 0) {
      return error("ReplayBlocks(): Catastrophic failure. Previous head is "
                   "also unknown or null. Please reindex.");
    }

    const CBlockIndex *pindexOldHead = mapBlockIndex[hashHeads[1]];

    if (!pindexOldHead || !pindexOldHead->IsValid(BLOCK_VALID_TRANSACTIONS)) {
      return error(
          "ReplayBlocks(): Previous head block is invalid. Please reindex.");
    }

    cache.SetBestBlock(pindexOldHead->GetBlockHash());
    cache.Flush();

    if (chainActive.Tip() &&
        chainActive.Tip()->GetBlockHash() != pindexOldHead->GetBlockHash()) {
      LogPrintf("ReplayBlocks(): Synchronizing active chain with rolled-back "
                "chainstate...\n");

      CValidationState state;
      int disconnectedBlocks = 0;

      while (chainActive.Tip() && chainActive.Tip()->GetBlockHash() !=
                                      pindexOldHead->GetBlockHash()) {
        CBlockIndex *pindexToDisconnect = chainActive.Tip();

        if (disconnectedBlocks > 1000) {
          return error("ReplayBlocks(): Too many blocks to disconnect during "
                       "recovery (%d). Please reindex.",
                       disconnectedBlocks);
        }

        if (pindexToDisconnect->nHeight <= pindexOldHead->nHeight) {
          break;
        }

        LogPrintf("ReplayBlocks(): Disconnecting block %s at height %d during "
                  "recovery\n",
                  pindexToDisconnect->GetBlockHash().ToString(),
                  pindexToDisconnect->nHeight);

        if (!DisconnectTip(state, params, nullptr)) {
          return error("ReplayBlocks(): Failed to disconnect block %s at "
                       "height %d during recovery: %s",
                       pindexToDisconnect->GetBlockHash().ToString(),
                       pindexToDisconnect->nHeight, state.GetRejectReason());
        }
      ...
      if (!chainActive.Tip() ||
          chainActive.Tip()->GetBlockHash() != pindexOldHead->GetBlockHash()) {
        return error("ReplayBlocks(): Failed to synchronize chain tip with "
                     "rolled-back chainstate. Expected %s, got %s",
                     pindexOldHead->GetBlockHash().ToString(),
                     chainActive.Tip()
                         ? chainActive.Tip()->GetBlockHash().ToString()
                         : "null");
      }

      LogPrintf("ReplayBlocks(): Successfully disconnected %d blocks during "
                "recovery\n",
                disconnectedBlocks);
    }

    std::vector<uint256> newHeads = view->GetHeadBlocks();
    if (newHeads.size() != 1 || newHeads[0] != pindexOldHead->GetBlockHash()) {
      return error(
          "ReplayBlocks(): Recovery failed - chainstate still inconsistent "
          "after rollback. Expected 1 head (%s), got %zu heads",
          pindexOldHead->GetBlockHash().ToString(), newHeads.size());
    }

    CValidationState flushState;
    if (!FlushStateToDisk(params, flushState, FLUSH_STATE_ALWAYS)) {
      return error("ReplayBlocks(): Failed to flush state after recovery: %s",
                   flushState.GetRejectReason());
    }

    LogPrintf("ReplayBlocks(): Recovery complete. Chain and chainstate "
              "synchronized at block %s, height %d.\n",
              pindexOldHead->GetBlockHash().ToString(), pindexOldHead->nHeight);
    return true;
  }
  ...
  const CBlockIndex *pindexNew = mapBlockIndex[hashHeads[0]];
    ...
    while (!blkdat.eof()) {
      ...
      try {
        ...
        if (mapBlockIndex.count(hash) == 0 ||
          ...
          LOCK(cs_main);
          CValidationState state;
          if (g_chainstate.AcceptBlock(pblock, state, chainparams, nullptr,
                                       true, dbp, nullptr))
            nLoaded++;
          if (state.IsError())
            break;
        ...
        while (!queue.empty()) {
          ...
          while (range.first != range.second) {
            ...
            if (ReadBlockFromDisk(*pblockrecursive, it->second,
              ...
              if (g_chainstate.AcceptBlock(pblockrecursive, dummy, chainparams,
...
bool LoadMempool(void) {
  ...
  try {
    ...
    while (num--) {
      ...
      if (nTime + nExpiryTimeout > nNow) {
        LOCK(cs_main);
        AcceptToMemoryPoolWithTime(chainparams, mempool, state, tx, nullptr,
                                   nTime, nullptr, false, 0);
        if (state.IsValid()) {
          ++count;
        } else {
  ...
  LogPrintf("Imported mempool transactions from disk: %i succeeded, %i failed, "
            "%i expired, %i already there\n",
