...
public:
  ...
  UniValue operator()(const WitnessUnknown &id) const {
    ...
    obj.push_back(
        Pair("witness_program", HexStr(id.program, id.program + id.length)));
    return obj;
  }
};
#endif
...
UniValue validateaddress(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 1)
    throw std::runtime_error(
        ...
        "watchonly\n"
        "  \"isscript\" : true|false,      (boolean, optional) If the address "
        "is P2SH or P2WSH. Not included for unknown witness types.\n"
        "  \"iswitness\" : true|false,     (boolean) If the address is P2WPKH, "
        "P2WSH, or an unknown witness version\n"
        "  \"witness_version\" : version   (number, optional) For all witness "
        "output types, gives the version number.\n"
        "  \"witness_program\" : \"hex\"     (string, optional) For all "
        "witness output types, gives the script or key hash present in the "
        "address.\n"
        "  \"script\" : \"type\"             (string, optional) The output "
        "script type. Only if \"isscript\" is true and the redeemscript is "
        "known. Possible types: nonstandard, pubkey, pubkeyhash, scripthash, "
        "multisig, nulldata, witness_v0_keyhash, witness_v0_scripthash, "
        "witness_unknown\n"
        "  \"hex\" : \"hex\",                (string, optional) The "
        "redeemscript for the P2SH or P2WSH address\n"
        "  \"addresses\"                   (string, optional) Array of "
        "addresses associated with the known redeemscript (only if "
        "\"iswitness\" is false). This field is superseded by the \"pubkeys\" "
        "field and the address inside \"embedded\".\n"
        "    [\n"
        "      \"address\"\n"
        "      ,...\n"
        "    ]\n"
        "  \"pubkeys\"                     (string, optional) Array of pubkeys "
        "associated with the known redeemscript (only if \"script\" is "
        "\"multisig\")\n"
        "    [\n"
        "      \"pubkey\"\n"
        "      ,...\n"
        ...
        "signatures required to spend multisig output (only if \"script\" is "
        "\"multisig\")\n"
        "  \"pubkey\" : \"publickeyhex\",    (string, optional) The hex value "
        "of the raw public key, for single-key addresses (possibly embedded in "
        "P2SH or P2WSH)\n"
        "  \"embedded\" : {...},           (object, optional) information "
        "about the address embedded in P2SH or P2WSH, if relevant and known. "
        "It includes all validateaddress output fields for the embedded "
...
UniValue createmultisig(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 2) {
    std::string msg =
        ...
        "transition to using addmultisigaddress to create multisig addresses "
        "with addresses known\n"
        "to the wallet before upgrading to v0.17. To use the deprecated "
        "functionality, start litecoincashd with "
        "-deprecatedrpc=createmultisig\n"
        "\nArguments:\n"
        "1. nrequired                    (numeric, required) The number of "
        "required signatures out of the n keys or addresses.\n"
        "2. \"keys\"                       (string, required) A json array of "
        "hex-encoded public keys\n"
        "     [\n"
        "       \"key\"                    (string) The hex-encoded public "
        "key\n"
        "       ,...\n"
        "     ]\n"
        ...
        HelpExampleCli("createmultisig",
                       "2 "
                       "\"["
                       "\\\"03789ed0bb717d88f7d321a368d905e7430207ebbd82bd342cf"
                       "11ae157a7ace5fd\\\","
                       "\\\"03dbc6764b8884a92e871274b87583e6d5c2a58819473e17e10"
                       "7ef3f6aa5a61626\\\"]\"") +
        "\nAs a json rpc call\n" +
    ...
    if (IsHex(keys[i].get_str()) && (keys[i].get_str().length() == 66 ||
      ...
      if (IsDeprecatedRPCEnabled("createmultisig") &&
        ...
        throw JSONRPCError(
            ...
            strprintf("Invalid public key: %s\nNote that from v0.16, "
                      "createmultisig no longer accepts addresses."
                      " Clients must transition to using addmultisigaddress to "
                      "create multisig addresses with addresses known to the "
                      "wallet before upgrading to v0.17."
                      " To use the deprecated functionality, start "
                      "litecoincashd with -deprecatedrpc=createmultisig",
                      keys[i].get_str()));
    }
  }
...
UniValue verifymessage(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 3)
    throw std::runtime_error(
        ...
        "\nAs json rpc\n" +
        HelpExampleRpc("verifymessage", "\"LEr4hNAefWYhBMgxCFP2Po1NPrUeiK8kM2\""
                                        ", \"signature\", \"my message\""));
...
UniValue logging(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() > 2) {
    throw std::runtime_error(
        ...
        "  - \"none\", \"0\" : even if other logging categories are specified, "
        "ignore all of them.\n"
        "\nArguments:\n"
        "1. \"include\"        (array of strings, optional) A json array of "
        "categories to add debug logging\n"
        "     [\n"
        "       \"category\"   (string) the valid logging category\n"
        "       ,...\n"
        "     ]\n"
        "2. \"exclude\"        (array of strings, optional) A json array of "
        "categories to remove debug logging\n"
        "     [\n"
        "       \"category\"   (string) the valid logging category\n"
        "       ,...\n"
        "     ]\n"
  ...
  if (changedLogCategories & BCLog::LIBEVENT) {
    if (!UpdateHTTPServerLogging(logCategories & BCLog::LIBEVENT)) {
      ...
      if (changedLogCategories == BCLog::LIBEVENT) {
        throw JSONRPCError(RPC_INVALID_PARAMETER,
                           "libevent logging cannot be updated when using "
                           "libevent before v2.1.1.");
      }
    }
  }
...
static UniValue getinfo_deprecated(const JSONRPCRequest &request) {
  throw JSONRPCError(RPC_METHOD_NOT_FOUND,
                     "getinfo\n"
                     "\nThis call was removed in version 0.16.0. Use the "
                     "appropriate fields from:\n"
                     "- getblockchaininfo: blocks, difficulty, chain\n"
                     "- getnetworkinfo: version, protocolversion, timeoffset, "
                     "connections, proxy, relayfee, warnings\n"
                     "- getwalletinfo: balance, keypoololdest, keypoolsize, "
                     "paytxfee, unlocked_until, walletversion\n"
                     "\nlitecoincash-cli has the option -getinfo to collect "
                     "and format these in the old format.");
}
