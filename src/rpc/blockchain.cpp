...
double GetDifficulty(const CChain &chain, const CBlockIndex *blockindex,
  ...
  if (getHiveDifficulty) {
    while (!blockindex->GetBlockHeader().IsHiveMined(consensusParams)) {
      if (!blockindex->pprev ||
          blockindex->nHeight < consensusParams.minHiveCheckBlock) {
        LogPrint(BCLog::HIVE,
                 "GetDifficulty: No hivemined blocks found in history\n");
        return 1.0;
      }
...
UniValue blockheaderToJSON(const CBlockIndex *blockindex) {
  ...
  bool isHive = blockindex->GetBlockHeader().IsHiveMined(consensusParams);

  result.push_back(Pair("type", isHive ? "hive" : "pow"));

  if (!isHive)
    result.push_back(
        Pair("powtype", blockindex->GetBlockHeader().GetPoWTypeName()));
  ...
  if (IsMinotaurXEnabled(blockindex, consensusParams))
    result.push_back(
        Pair("minotaurxdifficulty",
             GetDifficulty(blockindex, false, POW_TYPE_MINOTAURX)));
...
UniValue getdifficulty(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "difficulty.\n"
        "\nArguments:\n"
        "1. \"powalgo\":\"xxxx\"     (string, optional) This can be set to "
        "\"sha256d\" or \"minotaurx\". If omitted, wallet's default is assumed "
        "(-powalgo conf option)\n"

        "\nResult:\n"
        "n.nnn       (numeric) the proof-of-work difficulty as a multiple of "
  ...
  if (!algoFound)
    throw JSONRPCError(RPC_INVALID_PARAMETER,
                       "Invalid pow algorithm requested");

  if (!IsMinotaurXEnabled(chainActive.Tip(), Params().GetConsensus()) &&
      powType != POW_TYPE_SHA256)
    throw JSONRPCError(RPC_INVALID_PARAMETER,
                       "Non sha256d algo requested but minotaurx not enabled");
...
UniValue gethivedifficulty(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error("gethivedifficulty\n"
                             "\nReturns the Hive difficulty as a multiple of "
                             "the minimum difficulty.\n"
                             "\nResult:\n"
                             "n.nnn       (numeric) the Hive difficulty as a "
...
std::string EntryDescriptionString() {
  return "    \"size\" : n,             (numeric) virtual transaction size as "
         ...
         "in-mempool ancestors (including this one)\n"
         "    \"wtxid\" : hash,         (string) hash of serialized "
...
void entryToJSON(UniValue &info, const CTxMemPoolEntry &e) {
  ...
  info.push_back(
      Pair("wtxid", mempool.vTxHashes[e.vTxHashesIdx].first.ToString()));
...
UniValue getblockheader(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "provided)\n"
        "  \"type\" : \"hive\"|\"pow\", (string) Indicates whether this block "
        "is hive or pow mined\n"

        "  \"powtype\" : \"sha256d\"|\"minotaurx\"|\"unrecognised\", (string) "
        "Indicates the pow mining type of the block\n"
        ...
        "  \"minotaurxdifficulty\" : x.xxx,  (numeric) The pow difficulty for "
        "minotaurx (once activated)\n"
        ...
        "\nExamples:\n" +
        HelpExampleCli("getblockheader", "\"e2acdf2dd19a702e5d12a925f1e984b01e4"
                                         "7a933562ca893656d4afb38b44ee3\"") +
        HelpExampleRpc("getblockheader", "\"e2acdf2dd19a702e5d12a925f1e984b01e4"
                                         "7a933562ca893656d4afb38b44ee3\""));
...
UniValue getblock(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "  \"nonce\" : n,           (numeric) The nonce\n"
        "  \"bits\" : \"1d00ffff\", (string) The bits\n"
        "  \"difficulty\" : x.xxx,          (numeric) The pow difficulty for "
        "sha256d\n"
        "  \"minotaurxdifficulty\" : x.xxx, (numeric) The pow difficulty for "
        "minotaurx (once activated)\n"
        ...
        "}\n"
        "\nExamples:\n" +
        HelpExampleCli("getblock", "\"e2acdf2dd19a702e5d12a925f1e984b01e47a9335"
                                   "62ca893656d4afb38b44ee3\"") +
        HelpExampleRpc("getblock", "\"e2acdf2dd19a702e5d12a925f1e984b01e47a9335"
                                   "62ca893656d4afb38b44ee3\""));
...
static void ApplyStats(CCoinsStats &stats, CHashWriter &ss, const uint256 &hash,
  ...
  for (const auto output : outputs) {
    ...
    stats.nBogoSize +=
        32 + 4 + 4 + 8 + 2 + output.second.out.scriptPubKey.size();
  }
  ss << VARINT(0);
}
...
UniValue verifychain(const JSONRPCRequest &request) {
  ...
  return CVerifyDB().VerifyDB(Params(), pcoinsTip.get(), nCheckLevel,
                              nCheckDepth);
}
...
void BIP9SoftForkDescPushBack(UniValue &bip9_softforks,
  ...
  if (consensusParams.vDeployments[id].nTimeout > 0)
    bip9_softforks.push_back(Pair(VersionBitsDeploymentInfo[id].name,
                                  BIP9SoftForkDesc(consensusParams, id)));
}
...
UniValue getblockchaininfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "for sha256d\n"
        "  \"minotaurxdifficulty\": xxxxx, (numeric) the current difficulty "
        "for minotaurx once activated\n"

        "  \"mediantime\": xxxxxx,         (numeric) median time for the "
        "current best block\n"
        "  \"verificationprogress\": xxxx, (numeric) estimate of verification "
        "progress [0..1]\n"
        "  \"initialblockdownload\": xxxx, (bool) (debug information) estimate "
        "of whether this node is in Initial Block Download mode.\n"
        "  \"chainwork\": \"xxxx\"           (string) total amount of work in "
        "active chain, in hexadecimal\n"
        "  \"size_on_disk\": xxxxxx,       (numeric) the estimated size of the "
        "block and undo files on disk\n"
        "  \"pruned\": xx,                 (boolean) if the blocks are subject "
        "to pruning\n"
        "  \"pruneheight\": xxxxxx,        (numeric) lowest-height complete "
        "block stored (only present if pruning is enabled)\n"
        "  \"automatic_pruning\": xx,      (boolean) whether automatic pruning "
        "is enabled (only present if pruning is enabled)\n"
        "  \"prune_target_size\": xxxxxx,  (numeric) the target size used by "
        "pruning (only present if automatic pruning is enabled)\n"
        ...
        "           \"possible\": xx       (boolean) returns false if there "
        "are not enough blocks left in this period to pass activation "
        "threshold \n"
        "        }\n"
        "     }\n"
        "  }\n"
  ...
  if (IsMinotaurXEnabled(chainActive.Tip(), Params().GetConsensus()))
    obj.push_back(Pair("minotaurxdifficulty",
  ...
  for (int pos = Consensus::DEPLOYMENT_CSV;
       pos != Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++pos) {
    BIP9SoftForkDescPushBack(bip9_softforks, consensusParams,
                             static_cast<Consensus::DeploymentPos>(pos));
  }
...
UniValue mempoolInfoToJSON() {
  ...
  ret.push_back(Pair(
      "mempoolminfee",
      ValueFromAmount(std::max(mempool.GetMinFee(maxmempool), ::minRelayTxFee)
                          .GetFeePerK())));
  ret.push_back(
      Pair("minrelaytxfee", ValueFromAmount(::minRelayTxFee.GetFeePerK())));
...
UniValue getmempoolinfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "minimum mempool fee\n"
        "  \"minrelaytxfee\": xxxxx       (numeric) Current minimum relay fee "
        "for transactions\n"
        "}\n"
        "\nExamples:\n" +
...
UniValue getchaintxstats(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "transactions in the window. Only returned if \"window_block_count\" "
        "is > 0.\n"
        "  \"window_interval\": xxxxx,     (numeric) The elapsed time in the "
        "window in seconds. Only returned if \"window_block_count\" is > 0.\n"
...
static const CRPCCommand commands[] = {
    ...
    {"blockchain", "gethivedifficulty", &gethivedifficulty, {}},
