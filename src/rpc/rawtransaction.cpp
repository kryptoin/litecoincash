...
UniValue getrawtransaction(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
    throw std::runtime_error(
        ...
        "Note that if a blockhash is\n"
        "provided, only that block will be searched and if the transaction is "
        "in the mempool or other\n"
        "blocks, or if this node does not have the given block available, the "
        ...
        "3. \"blockhash\" (string, optional) The block in which to look for "
        "the transaction\n"
        ...
        "{\n"
        "  \"in_active_chain\": b, (bool) Whether specified block is in the "
        "active chain or not (only present with explicit \"blockhash\" "
        "argument)\n"
  ...
  if (!request.params[1].isNull()) {
    fVerbose = request.params[1].isNum() ? (request.params[1].get_int() != 0)
...
UniValue createrawtransaction(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
    throw std::runtime_error(
        ...
        "the litecoincash address, the numeric value (can be string) is the " +
        CURRENCY_UNIT +
        " amount\n"
  ...
  RPCTypeCheck(
      request.params,
      {UniValue::VARR, UniValue::VOBJ, UniValue::VNUM, UniValue::VBOOL}, true);
...
UniValue createrawbct(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 2 || request.params.size() > 4)
    throw std::runtime_error(
        "createrawbct [{\"txid\":\"id\",\"vout\":n},...] "
        "{\"address\":amount,\"data\":\"hex\",...} ( locktime ) ( replaceable "
        ")\n"
        "\nCreate a raw bee creation transaction spending the given inputs and "
        "creating the given number of bees.\n"
        "Does not add change address for unspent inputs, so be sure to add one "
        "as the final output using fundrawtransaction.\n"
        "Returns hex-encoded raw transaction.\n"
        "Note that the transaction's inputs are not signed, and\n"
        "it is not stored in the wallet or transmitted to the network.\n"
        "\nArguments:\n"
        ...
        "       ,...\n"
        "     ]\n"
        "2. bee_count              (numeric, required) The number of bees to "
        "create.\n"
        "3. \"honey_address\"        (string, required) The LCC address to "
        "receive rewards for blocks mined by bee(s) created in this "
        "transaction.\n"
        "4. community_contrib      (boolean, optional, default=true) If true, "
        "a small percentage of bee creation cost will be paid to a community "
        "fund.\n"
        "5. locktime               (numeric, optional, default=0) Raw "
        "locktime. Non-0 value also locktime-activates inputs\n"
        "\nResult:\n"
        "\"transaction\"             (string) hex string of the transaction\n"

        "\nExamples:\n" +
        HelpExampleCli("createrawbct",
                       "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" 1 "
                       "\"Cfkv9pniUJ2UoWvaukgD5Ksqx5EsVLzsCk\"") +
        HelpExampleCli("createrawbct",
                       "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\" 5 "
                       "\"Cfkv9pniUJ2UoWvaukgD5Ksqx5EsVLzsCk\" false") +
        HelpExampleRpc("createrawbct",
                       "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\", 12 "
                       "\"Cfkv9pniUJ2UoWvaukgD5Ksqx5EsVLzsCk\"") +
        HelpExampleRpc("createrawbct",
                       "\"[{\\\"txid\\\":\\\"myid\\\",\\\"vout\\\":0}]\", 34 "
  ...
  RPCTypeCheck(request.params,
               {UniValue::VARR, UniValue::VNUM, UniValue::VSTR, UniValue::VBOOL,
                UniValue::VNUM},
               true);
  if (request.params[0].isNull() || request.params[1].isNull() ||
      request.params[2].isNull() || request.params[2].get_str().empty())
    throw JSONRPCError(
        RPC_INVALID_PARAMETER,
        "Invalid parameter, arguments 1, 2 and 3 must be non-null");
  ...
  if (!IsValidDestination(destinationFCA)) {
    throw JSONRPCError(RPC_INVALID_PARAMETER,
                       "Invalid parameter, Invalid honey address specified");
  }
  ...
  if (!Solver(GetScriptForDestination(destinationFCA), whichType, vSolutions)) {
    throw JSONRPCError(
        RPC_INVALID_PARAMETER,
        "Invalid parameter, Couldn't solve scriptPubKey for honey address");
  }
  if (whichType != TX_PUBKEYHASH) {
    throw JSONRPCError(RPC_INVALID_PARAMETER,
                       "Invalid parameter, Honey address must be legacy format "
                       "(TX_PUBKEYHASH)");
  }
...
UniValue decoderawtransaction(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
    throw std::runtime_error(
        ...
        "2. iswitness          (boolean, optional) Whether the transaction hex "
        "is a serialized witness transaction\n"
        "                         If iswitness is not present, heuristic tests "
  ...
  if (!DecodeHexTx(mtx, request.params[0].get_str(), try_no_witness,
                   try_witness)) {
...
static const CRPCCommand commands[] = {
    ...
    {"rawtransactions",
     "createrawbct",
     &createrawbct,
     {"inputs", "beecount", "honey_address", "community_contrib", "locktime"}},
    ...
    {"rawtransactions",
     "signrawtransaction",
     &signrawtransaction,
     {"hexstring", "prevtxs", "privkeys", "sighashtype"}},
