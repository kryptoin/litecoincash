...
UniValue getpeerinfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        "to addnode/-connect or if it was an automatic/inbound connection\n"
...
UniValue getaddednodeinfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() > 1)
    throw std::runtime_error(
        ...
        "connected = true\n"
        "       {\n"
        "         \"address\" : \"192.168.0.201:9333\",  (string) The "
        "litecoincash server IP and port we're connected to\n"
...
UniValue getnetworkinfo(const JSONRPCRequest &request) {
  if (request.fHelp || request.params.size() != 0)
    throw std::runtime_error(
        ...
        CURRENCY_UNIT +
        "/kB\n"
        "  \"incrementalfee\": x.xxxxxxxx,          (numeric) minimum fee "
        "increment for mempool limiting or BIP 125 replacement in " +
        CURRENCY_UNIT +
        "/kB\n"
